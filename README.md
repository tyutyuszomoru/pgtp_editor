...
- **The sandbox and the check ladder.** A local, disposable PostgreSQL sandbox (provisioned from **New Project** or **Project Settings ▸ Connections**, optionally cloned with data) runs a layered check: parse, apply, then `plpgsql_check`. Findings land as clickable `[Check]` rows. A **Sandbox SQL Console** tab runs ad-hoc SQL — sandbox-only, structurally, with a row cap that reports truncation as a fact rather than as a short grid.
...
- **A Quality SQL Console — ad-hoc read/write SQL against the production database.** Specified, not yet built, and called out here because it deliberately reopens part of the gap described above: it is a full console pointed at the live database, always available once a quality connection exists, chosen over a read-only variant and over a maintenance-mode gate with the hazard stated. It is a **second, separately marked** console — the sandbox one stays structurally unable to reach anything else. **One guard was kept, deliberately and in its strong form: nothing it runs commits until you say so.** A Run executes inside an open transaction, you see what happened while it is still undone, and an explicit commit gesture is what makes it real — which is *further* than DBeaver in the safe direction rather than a retreat from it. That was the point: per-statement commit against production would have imported the exact behaviour this tool exists to replace, onto the surface where it does the most damage. The read/write hazard is still real and still stated in the spec; what is not true is that nothing about the console is on your side.
