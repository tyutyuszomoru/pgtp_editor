...
- **DDL Explorer.** Read-only introspection through `pg_catalog` into one synthesized DDL buffer plus a cross-referenced tree (Tables, and Functions & Procedures; triggers appear under both). The buffer holds every function, procedure and trigger definition, folded per object, and clicking a tree item jumps to it. Two instances can be open at once — **DDL Explorer (Quality)** and **DDL Explorer (Sandbox)**.
...
- **A Quality SQL Console — ad-hoc read/write SQL against the production database.** Specified, not yet built, and called out here because it deliberately reopens part of the gap described above: it is a full console pointed at the live database, always available once a quality connection exists. The owner chose it over a read-only variant and over a maintenance-mode gate, with the hazard stated. It is a **second, separately marked** console — the sandbox one stays structurally unable to reach anything else — but nothing about it pretends to be safe, and the specification says so rather than dressing it up.
...
- **Table, view and constraint DDL in the DDL Explorer.** The read-only pane shows routines and triggers today; the plan is to synthesize `CREATE TABLE` for tables and to show view and matview definitions beside them, with constraints, foreign keys and indexes as tree items that jump to their own DDL. Postgres has no `pg_get_tabledef`, so a synthesized table definition is a *reconstruction* — and it will say so in the buffer rather than let a reader mistake it for the whole truth.
