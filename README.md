...
- **An editing-primitive layer in every editable editor, specified as vim's command grammar.** `Esc` puts an editable editor into a **Command mode** beside the ordinary **Edit mode** — per editor, transient, nothing persisted, no setting to switch on. There you get what no Windows editor and no menu can express: relative count-motions (`42j` — "go down 42 lines"), motions by word, line, paragraph, character and bracket, and delete/change/yank by any of them. **Edit mode gains nothing** — that is the point; the choice was to adopt a standard vocabulary or invent a parallel keymap of Ctrl-chords, and the standard one won. `:` addresses the app's **own menu commands** rather than inventing a second vocabulary, so it stays in step with the menus by construction. One system clipboard, no registers. A gesture that cannot run says why.
...
- **Closing the last two gaps in a reconstructed `CREATE TABLE`.** Table DDL in the Explorer now covers columns, identity/`SERIAL`, `GENERATED` columns, constraints, indexes and comments; **table inheritance and partitioning are deliberately still open**, because closing them restructures the statement rather than extending it. That is a decided boundary, not an unfinished job — it closes when a feature actually consumes that buffer as more than something to read, and until then nothing treats it as a deployment source.
- **Vim text objects** (`ciw`, `di"`, `ci(`) are the editing layer's deferred half. They are the one part that needs a language model, so they will be plpgsql-only and will state their unavailability elsewhere rather than fall back to a character guess.
...
7585 passing / 51 skipped at the last full-suite run (commit `6258349`), recorded in [`docs/TEST_LOG.md`](docs/TEST_LOG.md) alongside every earlier one, spanning the model, diff, schema-learning, validation, SQL analysis, database, generation and UI layers. That log is the committed record of verified runs, and this line cites it rather than a number of its own — a count nothing verifies is stale by the next commit.
