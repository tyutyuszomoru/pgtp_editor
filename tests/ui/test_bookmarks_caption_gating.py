"""§8/§13 — the Bookmarks menu is disabled during Caption Mode.

Caption Mode makes the Raw XML editor read-only, but the Bookmarks menu and its
...
def test_the_lane_retains_its_menu_and_four_actions(qtbot, tmp_path):
    ...
    assert window._find_ui.bookmarks_menu is menu
...
def test_bookmarks_are_enabled_outside_caption_mode(qtbot, tmp_path):
    window = _window(qtbot, tmp_path)
    assert find_top_menu(window, "Bookmarks").isEnabled() is True
...
def test_entering_caption_mode_disables_the_menu_and_every_action(qtbot, tmp_path):
    ...
    assert find_top_menu(window, "Bookmarks").isEnabled() is False
    for action in window._find_ui.bookmark_actions:
...
def test_set_bookmarks_enabled_is_safe_before_the_menu_is_built(qtbot):
    """The host may gate before `build_bookmarks_menu` has run."""
    ...
    assert controller.bookmarks_menu is None
