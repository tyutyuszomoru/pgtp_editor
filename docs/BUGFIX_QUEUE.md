...
**Status:** OPEN — facet B RESOLVED (ec03946); **facet A re-triaged 2026-08-06 and still OPEN**. Facet B's fix also cured an unreported sibling: `kind="reference"` rows had no `_ROW_BUILDERS` entry either, so anything under a relation's "Referenced by" group crashed too. Those rows now degrade to an EMPTY Properties panel where the retired Table References panel showed the owning node — a behavior change awaiting a decision, since `TableReference.kind` (`"page"|"detail"|"column"`) is already on the node's payload and `_reference_node` (`db/coherence.py:246`) simply drops it.
