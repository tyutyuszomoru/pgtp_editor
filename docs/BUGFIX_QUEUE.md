...
## BUG-260811232724: clicking a routed audit item — a Find All finding OR a bookmark — does not navigate the read-only DDL Explorer EditorPanel to its location; the buffer is deliberately routed as `_NO_AUDIT_ROUTE`
**Status:** OPEN
**Reported:** 2026-08-11
...
**Root cause:** `_bookmark_audit_route` returns the sentinel `_NO_AUDIT_ROUTE` (`ui/find_controller.py:147`, `:811-821`) for the read-only DDL Explorer buffer (`editor is panel.editor` for a `stage.ddl_explorer_panels()` role). A `_NO_AUDIT_ROUTE` row is emitted **roles-less** on purpose — no `UserRole` line, no `UserRole+1` target (`:682-689` for Find All, `:761-765` for bookmarks) — so `_on_audit_item_clicked` returns immediately at its `if line is None` guard (`main_window.py:2503-2505`). The row is therefore inert by construction: clicking never reaches any jump.

**Reference path (owner-confirmed: "it works in xml").** Clicking a finding/bookmark whose source is the Raw XML editor navigates correctly — the mechanism is sound — because that row carries a real payload: `UserRole = line` and `UserRole+1 = "raw"`, and `_on_audit_item_clicked`'s fallback (`main_window.py:2529-2530`) does `center_stage.setCurrentIndex(raw_xml_tab_index); xml_editor.navigate_to_line(line)`. The DDL Explorer route lacks **exactly two things** the raw route has, and nothing else: (1) a **route-table entry** — `_bookmark_audit_route` returns `_NO_AUDIT_ROUTE` instead of a real `(target, line-bearing)` payload, so the row is emitted with no line and no target (this is the whole failure — there is no offset→line mapping problem, the match line is already 1-based and correct, and no signal is disconnected: `itemClicked → _on_audit_item_clicked` is wired at `main_window.py:1062-1063` and fires); and (2) a **matching branch in `_on_audit_item_clicked`** that resolves that target to the read-only `EditorPanel` (the raw case is handled by the fallback; the DDL Explorer needs its own branch because the fallback hard-codes the Raw XML editor). Add both and the DDL Explorer path becomes a mirror of the working raw path.
