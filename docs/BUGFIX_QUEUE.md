...
**Proposed fix — Part A (run the SQL on quality even on identity mismatch):** The change is in `_precondition_signature` (`pgtp_editor/ui/ddl_object_editor.py:1338-1396`) — the ONLY place the quality path diverges from sandbox. The owner's ruling is "trust the user, run the sql," and the sandbox path is the exact precedent for what that looks like (no signature gate at all). Two shapes, pick per the confirmation question below:
- **Preferred — demote the hard refusal to a confirm-gated warning, do NOT run silently.** In the mismatch branch (`:1384-1395`), instead of `return False`, surface the same mismatch text through the existing `self._confirm(...)` seam (the one already used at `apply_to_target` `:1305` and `_precondition_validation` `:1427`) titled in the `GESTURE_APPLY_TO_QUALITY` vocabulary (`GESTURE_LABELS[GESTURE_APPLY_TO_QUALITY]`, "Apply to quality"), warning that the buffer's identity differs from the live object and that applying will CREATE A SECOND OBJECT and leave the old one live (reuse the wording already at `:1387-1392`). If the user confirms, `_report` a "proceeding despite identity mismatch" line (mirroring `_precondition_validation`'s override at `:1445-1448`) and `return True` so the write seam runs; if declined, `_report` the cancellation and `return False`. The other branches of `_precondition_signature` (unparseable buffer `:1348-1356`, live-identity lookup FAILURE `:1367-1375`, object-absent `:1376-1383`) should stay REFUSALS — those are "we could not rule out the hazard / connection error," not "the user chose a rename." Do NOT relax the connection-error branch: turning an unreachable database into a cleared precondition is the one failure mode the docstring at `:1357-1363` (FQ-009) explicitly guards, and the owner's "trust the user" is about the user's *intent*, not about swallowing a driver error.
- **Alternative — remove the gate entirely (sandbox parity, least code).** Delete the mismatch refusal so `apply_to_target` runs like `apply_to_sandbox`. This is the most literal reading of "run the sql," but it drops the last warning before an irreversible real-DB write with no revert snapshot (precondition 3), so a mis-typed name silently forks a function in *quality*. Given the report explicitly frames this as "an edge case… it contradicts checkout," the confirm-gated warning (preferred shape) honors "trust the user" (they can always proceed) while not making the destructive real-DB write utterly unguarded. **Flag for owner if unsure: does "run the sql" mean run-with-a-warning-confirm or run-with-nothing?** The rest of `apply_to_target` already confirms twice (validation override `:1427`, the apply confirmation `:1305`), and a destructive-op confirm is the standard elsewhere in this file — so the preferred shape is consistent with the codebase; but the owner's literal words are "run the sql," so if a background ruling is wanted, file `owner-decision`. Implement the preferred (warn-then-run) shape by default and state the choice in the commit message.
- **Non-negotiable regardless of shape:** the path must NEVER silently no-op. Every branch either runs the seam or emits a `[Check]` line saying exactly why it did not. That invariant is already mostly there; the fix must preserve it.
...
**Test impact:**
- Part A: `tests/ui/test_ddl_object_editor.py` owns the precondition contract. The case that pins the CURRENT (buggy) behavior is `test_precondition_1_refuses_a_changed_signature_naming_the_mismatch` (`:1031-1046`) — it asserts `panel.apply_to_target() is False` on a renamed buffer, which the fix INVERTS. Rewrite it: with the confirm-gate accepted (`stub_confirm=True`) a renamed buffer now runs the seam and returns `True` and emits the "proceeding despite identity mismatch" line; add a companion asserting that DECLINING the mismatch confirm returns `False` and reports the cancellation. Keep `test_precondition_1_refuses_a_buffer_whose_signature_cannot_be_parsed` (`:1054`) and the connection-failure/absent cases UNCHANGED — those branches stay refusals under the preferred shape. Reuse the existing `_FakeSeams`/`apply_to_target` harness (`:810-831`).
