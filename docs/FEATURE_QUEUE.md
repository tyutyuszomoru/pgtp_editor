...
**Problem:** The SQL editors have no way to select structural units by keyboard. Today `Select Enclosing Block` (Ctrl+Shift+B) is capability-dispatched (`ui/main_window.py`, `_build_select_menu` ~:2508, entries ~:2550-2557, dispatch `_select_enclosing_block` ~:2564/:2596-2601) — on a `CodeEditor` tab it only does `select_enclosing_brackets` (innermost `()[]{}`, `ui/code_editor.py:412`), and `Select Parent Block` (Ctrl+Shift+A, `_select_parent_block` ~:2603) is XML-only and hidden on code tabs. So selecting "this statement", "this CASE", "this LOOP body", "the whole BEGIN…END" is all manual mouse work. This is the plpgsql analog of the XML editor's existing structural selection, and its absence makes structural edits (delete a CASE, reindent a LOOP, comment a statement) slow.

**Proposed approach:**
- **Interaction = ONE repeatable expand key + a shrink counterpart** (owner-chosen over a two-command model). Each press of the expand key grows the selection to the next-larger enclosing structural unit; the shrink key steps it back down. State = a per-selection expansion stack. Grow bottoms at the word/identifier at the caret and tops at the whole `BEGIN…END` body.
- **The ladder is a RECURSIVE CLIMB, not a fixed 5 levels:** word/parameter → current `()`/expression group → up-to-next-keyword (clause) → current statement (between `;`/keyword boundaries) → climb each enclosing block one press at a time (an `IF` inside a `FOR` inside a `BEGIN` is three block-climbs) → whole `BEGIN…END` body. Within `CASE`: current `WHEN…THEN` branch → the whole `CASE`. (The paren/expr level and the CASE-branch sub-level are owner-approved additions beyond the original five.)
- **Editors:** the SQL editors — the DDL object editor (`ui/ddl_object_editor.py`) and the Sandbox SQL Console (`ui/sql_console_panel.py`). Not the XML editors (they keep their own XML structural selection).
- **Keybinding (DEC-012-clean, family-dispatched):** make **Ctrl+Shift+A the repeatable structural-expand on the SQL editors** — it is free on code tabs today (parent-block is hidden there); on XML tabs it stays `Select Parent Block` (same chord, family-dispatched, exactly the pattern Ctrl+Shift+B already uses; DEC-012 at `CONSOLIDATED_SPEC.md:2641`). The existing Ctrl+Shift+B bracket-select on code tabs becomes **one rung** of the expand ladder. **Post-BUG-046 reality (verified):** the old duplicate Ctrl+Shift+B handler in `CodeEditor.keyPressEvent` is already gone (BUG-046 owner ruling 2026-08-10, `main_window.py:2579-2594`) — Ctrl+Shift+B is now the sole menu-hosted, FQ-012-rebindable action; build on that. The **shrink** counterpart gets a sane default chord (owner left the exact chord open), rebindable via the shipped FQ-012 Customize-Shortcuts.
- **The reuse crux (anti-fork):** build a NEW **Qt-free block/clause SPAN model in `sql/`** (a span tree keyed by `(kind, start, end, depth)` — kinds word/paren/clause/statement/begin/if/loop/case/when/declare), placed BESIDE FQ-030's shipped `sql/from_clause.py`/`sql/routine_scope.py` (§18.9). Reuse: `sql/tokenizer.py::tokenize` (the ONE lexer — no second lexer), `sql/statements.py` (statement spans + `statement_at`, already wired), `ui/code_editor.py::enclosing_bracket_span` (`:107`, the paren rung — wire in, don't reimplement), the caret-at-start selection idiom (`code_editor.py:422-427`), and trigger-time editor resolution via `active_selection_editor()` (`main_window.py:2596`). **Lift the block-balance RULES from `sql/formatter.py::_Reindenter`** (`_BLOCK_STARTERS` ~:78, `_BREAK_AFTER` ~:81, `_open_frames` ~:510, `_close_block` ~:600, the `BEGIN/IF/LOOP/CASE → END/END IF/END LOOP/END CASE` matching) — do NOT fork a second block-balance walk. The formatter's frame stack is throwaway (drives indentation/refusal only, exposes no spans), so it can't be consumed as-is; lifting the shared rule tables is the mandate and the single highest fork risk. NEVER parse SQL in `ui/`. Coverage today: tokens/parens/statements consumable; clause-spans and block-spans are the genuinely new work.

**Alternatives considered:** a two-command model (enclosing + parent, mirroring XML) — rejected by the owner in favor of one repeatable expand + shrink (matches expand-region UX and the "then, then, then" description). A third independent SQL parser for the span model — rejected as the classic fork; lift the formatter's rules onto the shared tokenizer/statements layer instead.

**Suggested placement:** EXTEND, split precisely — **§8** (the editor section owning the `Select` menu + Select Enclosing/Parent Block, `CONSOLIDATED_SPEC.md:2565-2639`, DEC-012 at :2641) owns the UX, menu wiring, and keybinding; **§18.9 / `sql/`** (beside `from_clause.py`/`routine_scope.py`) owns the new Qt-free span model. NOT a new top-level section, NOT in `ui/`. Do NOT conflate with FQ-030's "expand-SELECT" (a schema-dynamic TEMPLATE expansion, `code_editor.py:429`/`sql/templates.py`) — different mechanism, same word. FQ-032's deferred vim text-objects are a FUTURE consumer of this same span model (keep the structural span model orthogonal to FQ-030's semantic scope model — do not fuse into one god-analyzer).

**Open questions:** (1) exact default chord for shrink (rebindable via FQ-012 regardless); (2) whether Ctrl+Shift+B bracket-select stays as a direct shortcut in addition to being a ladder rung, or is fully absorbed; (3) exact schema of the span model and how the WHEN-branch sub-level is represented; (4) caret-inside-opaque-region behavior (`$$` body / string / comment) — presumably the whole opaque token is one unit; confirm at design time.
...
**Problem:** Today the New Project dialog (`ui/new_project_dialog.py::NewProjectDialog`, fields ~:94-334) collects only identity (name/description), the project folder, the **sandbox** connection (host/port/user/pass + superuser Test, provisioning mode — FQ-007), and a git placeholder. It has **no `.pgtp` field and no target/quality connection section at all** — the `.pgtp` is linked only later (on open, BUG-021) and the target/quality connection is configured only later in Project Settings (auto-imported from the `.pgtp`'s `<ConnectionOptions>` on first load). So a user setting up a real project must do it in three disjoint steps across time (create → open the `.pgtp` → configure quality), which obscures how the pieces relate. This is a UX/onboarding convenience that collapses those into one coherent New Project pass.

**Proposed approach (mostly surfaces mechanisms that already exist):**
- Add an **optional "Attach `.pgtp`…" open-file field** to the New Project dialog. Leaving it empty = today's behavior unchanged (sandbox-only project).
- Attaching a `.pgtp` **establishes the link at creation** via the existing `db/ddl_project.py::PgtpLink` (`source_path`/`working_copy_path`/`last_known_source_checksum`), i.e. do at creation what opening a `.pgtp` into a project already does today — no new link model.
- Attaching a `.pgtp` **reveals a quality-server (target) section pre-populated from its `<ConnectionOptions>`**, using the already-shipped parser `db/config.py::connection_from_tree` (host/port/database/user; password is blank/obfuscated in the XML, so auto-fill is everything-but-password). This is the exact population that already happens automatically on document load (into `ProjectSettings.target`) — this feature just brings it FORWARD into the dialog. The section mirrors Project Settings' target section (host/port/db/user/pass + the FQ-001 generic-connectivity **Test** button reusing `db/introspect.test_connection`), so the user types the password and verifies the quality server before creating. The section stays hidden when no `.pgtp` is attached (nothing to populate it, nothing to check against).
- Net: sandbox + quality + `.pgtp` link are all set up in one pass; on confirm, creation writes the `PgtpLink` and the `target` connection into `settings.json` alongside the existing sandbox provisioning (FQ-007).

**Alternatives considered:** reveal a BLANK quality section (no auto-fill) — rejected by the owner; auto-populating from the `.pgtp` is the whole "do everything instantly" payoff and reuses `connection_from_tree`. Leave the flow as three separate steps (status quo) — rejected as the exact UX opacity this fixes.

**Suggested placement:** EXTEND **§18.2** ("Projects, checkout & state markers", `CONSOLIDATED_SPEC.md` ~:5270-5420, which documents the New Project dialog collecting folder + sandbox + git today). Reuse verbatim: `PgtpLink` (link model), `connection_from_tree` (parse `<ConnectionOptions>`), the Project-Settings target-connection form + FQ-001 Test button, and the FQ-007 creation/provisioning flow (`ui/ddl_project_controller.py::create_project` ~:308-324). No new subsystem — this is surfacing existing mechanisms earlier in the flow.
...
**Status:** PROCESSED (spec §26; `pgtp_editor/version.py` + About box; bump rule in CLAUDE.md "Versioning", commit 3adc34c)
**Closing note:** shipped with the resolution order INVERTED from this entry's title — `pyproject.toml` is read FIRST and `importlib.metadata` only as fallback. The entry assumed package metadata was the truth; in an editable install it goes stale, and this very checkout proved it (metadata reported `0.3.0` while `pyproject.toml` said `0.4.0`), so metadata-first would ship a wrong version in the one environment where anyone can check it. A test pins the order and counts calls, so an implementation that resolves both and prefers metadata cannot pass. The `.iss` half of activity (3) needed no work: `docs/installer.iss` already scanned `pyproject.toml` at compile time and `#pragma error`s if the literal is missing — so bumping a version has never required touching it, and the CLAUDE.md rule now says so rather than repeating the entry's assumption. The frozen PyInstaller path remains **UNVERIFIED** — `--copy-metadata` is passed behind an availability probe that no build environment has ever exercised.
**Requested:** 2026-08-10
...
**Problem:** There are two live DDL Explorer instances (§18.7 / FQ-022): the **quality/target** one — edits/checkouts touch the real production/target DB, so it is the dangerous surface — and the **sandbox** one, disposable and read-only. Today they render **identically**: the only distinctions are the menu-entry rename to "DDL Explorer (Quality)" (FQ-022, spec ~:2150/:9927) and the sandbox's read-only menu. There is **no at-a-glance visual cue** that the quality explorer is the dangerous one — both trees use the same global blue selection highlight, so a user can forget which explorer they are acting in. This adds a color cue to the textual one already in place.

**Proposed approach (grounded, file:line verified this session):**
- Scope: the **quality/target DDL Explorer only** — `main_window.py:785` `self.ddl_browser_panel = BrowserPanel()` (the `browse_only=False`, role `DDL_EXPLORER_TARGET` / "target" instance, `_ddl_explorers` dict `main_window.py:843-848`). Change its tree's **selected-row highlight to RED/rose**. The **sandbox** instance (`main_window.py:830` `BrowserPanel(browse_only=True)`, role "sandbox") keeps normal blue — unchanged.
- Color derivation, "same contrast ratio, red instead of blue": the current highlight is `QPalette.Highlight = #3874f2` (blue) in BOTH themes — verified `ui/theme.py:49` (light) and `:86` (dark) — with white `HighlightedText` (`:50` / `:87`), rendered against tree `Base` `#FFFFFF` (light, `ui/theme.py:41`) / `#1E1E1E` (dark, `:78`). Derive a **red at matching relative luminance / WCAG contrast** against each theme's Base so the red reads with the same prominence the blue does — a **per-theme value** (the blue's contrast already differs by theme because the backgrounds differ; match that per theme). Keep `HighlightedText` white. Hue family = rose/red (danger); implementer picks the exact hex at matching luminance per theme.
- Mechanism: the DDL Explorer tree is a `QTreeWidget` (`ddl_buffer_panel.py:501`) whose highlight is purely global-palette-driven (no per-widget stylesheet/delegate/paintEvent today). Apply a **per-widget palette override** on the quality tree setting only the `Highlight` role to the derived red — reuse the shipped precedent at `ui/sql_results_panel.py:545-550` (get palette → set role → `setPalette()`, confirmed this session).
- **Theme-toggle re-apply (the one real wrinkle):** `apply_theme()` (`ui/theme.py`) calls `app.setPalette(...)` on light/dark toggle, which **wipes per-widget overrides**, so the quality-explorer red must be **re-derived and re-applied on theme change** — hook `_on_light_theme_toggled()` (`main_window.py:1952-1959`) or override `changeEvent(QEvent.PaletteChange)` on `BrowserPanel`, mirroring the existing toolbar `refresh_icons()` theme-change hook (`main_window.py:1893` / `:1956`). Stated explicitly so the implementer does not ship a red that reverts to blue after the first theme switch. (Note: `ui/theme.py` line numbers cited from a `ui/theme.py:49/:86` read; `apply_theme()`/`_on_light_theme_toggled()` line refs are as supplied by the owner — implementer should confirm the exact lines.)

**Alternatives considered:**
- *A broader danger affordance* (red header bar / border / badge on the quality explorer) instead of the line highlight — rejected as the primary change: the owner specifically asked for the **line/selection highlight**, and it is the smallest, clearest change. A header/badge is worth noting only as a **possible complement**, not a substitute. The menu rename "DDL Explorer (Quality)" already provides the textual distinction; this adds the at-a-glance color one.
- *Changing the global theme `Highlight`* — rejected: that would recolor selection everywhere in the app. This must be a per-widget override scoped to the one tree, which is why the `sql_results_panel.py` setPalette precedent (not a `theme.py` edit) is the right mechanism.

**Suggested placement:** EXTEND **§18.7** ("Two live DDL Explorer instances — target vs sandbox", `CONSOLIDATED_SPEC.md:9834`) — it defines the two instances but currently specifies **no** visual differentiation; this adds the danger cue it lacks. Secondarily touches the **theming conventions (§7/§8, wherever `ui/theme.py` and per-widget palette overrides are documented)** because it introduces the **first per-widget `Highlight` override that must survive theme switches** — that theme-toggle re-apply contract should be recorded where theming lives. Reuse: the `ui/sql_results_panel.py:545-550` `setPalette` precedent and the toolbar `refresh_icons()` theme-change-hook precedent (`main_window.py:1893`/`:1956`). Not a new section.

**Open questions (non-blocking, design-time):** (1) the exact red/rose hex per theme — computed at matching relative luminance vs. each Base, or hand-picked to that target; (2) whether the quality tree's **inactive-selection / hover** states also need the red (not just the active `Highlight` role); (3) whether `HighlightedText` stays white or needs a tweak for legibility on the chosen red.
...
**Problem:** The DDL Explorer (`ui/ddl_buffer_panel.py::BrowserPanel`, a `QTreeWidget`; tree built in `set_schema()` ~:528-561 via `_build_tables_branch` ~:619 and `_build_routines_branch` ~:694) has **no find/filter at all today** (zero filter/find/`setHidden` in the file). On a real database the two branches (Tables; Functions & Procedures) hold many objects, so finding one means scrolling — there is no way to narrow the tree to objects whose name matches a term.

**Proposed approach (grounded, file:line verified):**
- Add a **filter row above the tree** in `BrowserPanel`'s `QVBoxLayout` (currently just `layout.addWidget(self.tree)` at `ddl_buffer_panel.py:524-526`): an **input field**, a **match-mode `QComboBox`**, a **"Filter" button**, and a **"Clear Filter" button**.
- **Match modes (a NEW predicate set):** Contains · Starts with · Doesn't contain · Doesn't start with · Ends with. Mirror the label↔value dropdown SHAPE of `ui/caption_management_panel.py::MODE_LABELS` (~:79-83), but define a **new predicate enum** — these modes are about WHERE/how a name matches, whereas caption's `normal/extended/regex` (`caption_scan.py::SEARCH_MODES` ~:244) are about how the search STRING is interpreted. Do **not** reuse or conflate the caption enum.
- **Apply on the Filter button** (not live-as-you-type — owner specified explicit buttons); **Clear Filter** restores all rows.
- **Match target = DDL OBJECT NAMES** (owner-clarified): the object-level node labels — **tables, routines (functions/procedures), and triggers** (`schema.table`, `schema.name [P|F|T]`, `schema.table.trigname [timing][events]`; formats at `ddl_buffer_panel.py:648/721/765`). **NOT** columns or argument leaves, and **NOT** the DDL source text.
- **Tree semantics = keep ancestors of matches visible** (owner-confirmed): show a matching object AND its parent branch/table so the hit shows in context; hide a branch/table/routine that has no matching descendant. Implement via `item.setHidden(...)` (reuse the precedent at `caption_management_panel.py:631`, `item.setHidden(not matched)`) walking the tree, so branch roots ("Tables"/"Functions & Procedures") and parent table/routine nodes stay visible iff they contain a match. Columns/args ride along (visible) under a shown parent but are never match targets.
- **Case-insensitive** matching recommended for search-ease (flag as a settable detail — see OQ 1).
- **Appears in BOTH DDL Explorer instances** — quality (`main_window.py:785` `BrowserPanel()`) and sandbox (`:830` `BrowserPanel(browse_only=True)`) are the same class; the filter is a harmless search aid in both. It is not a context menu, so `browse_only` does not gate it.
- **Purely a visibility filter — changes NOTHING about edit behavior** (owner's explicit guard): hidden rows are just hidden; visible rows keep their exact current Edit DDL / checkout / context-menu behavior. No change to selection, navigation, or the edit pane.
- Optional polish (reuse): the Coherence panel's active-filter BANNER + Clear pattern (`ui/coherence_panel.py` ~:217-236/:319-326, `filter_banner_label`/`clear_filters_button`/`filter_description()`/`NO_MATCHES_TEXT` ~:118) — a small "filter active: <mode> '<term>'" indicator and an all-hidden "no matches" message. Cheap and consistent; not required by the owner (see OQ 2).

**Alternatives considered:**
- *Live-as-you-type filtering (no buttons)* — rejected: the owner explicitly specified a Filter button + Clear Filter button (apply-on-demand).
- *A `QSortFilterProxyModel` (like the Caption panel)* — rejected: the DDL Explorer is a `QTreeWidget` (item-based, not model/view), so `item.setHidden` walking the tree is the fitting mechanism and preserves ancestor visibility straightforwardly; a proxy model would be a larger refactor for no gain.
- *Matching the DDL source text or columns/args* — rejected by the owner: "DDL objects" = object names; columns/args excluded.

**Suggested placement:** EXTEND **§18.1** (the DDL Explorer section, `CONSOLIDATED_SPEC.md` ~:4943-5242) — add the name-filter bar; since it lives on `BrowserPanel` it appears in both instances, so **cross-reference §18.7** (two instances, ~:10053-10233, "Two live DDL Explorer instances"). Neither section mentions any find/filter today, so this is net-new UI on an existing panel — not a new top-level section. Reuse: `item.setHidden` (caption precedent, `caption_management_panel.py:631`), the caption `MODE_LABELS` dropdown SHAPE (`caption_management_panel.py:79-83`, with a NEW predicate enum), and optionally the coherence active-filter-banner pattern.

**Open questions (non-blocking, design-time):** (1) case-sensitivity default (recommend insensitive) and whether to expose a toggle; (2) whether to show the coherence-style active-filter banner + no-matches message; (3) default dropdown mode (recommend "Contains"); (4) behavior when the tree is refreshed/reparsed (`set_schema` rebuilds the tree wholesale, `ddl_buffer_panel.py:542`) while a filter is active — re-apply the current filter to the rebuilt tree, or clear it? (recommend re-apply; note the tree is rebuilt whole, so a filter would need to survive the rebuild the way `_dirty_keys` does at ~:522/:558-561); (5) the owner called this "function 1" — a later "function 2" (e.g. a non-filtering find/jump) may extend the same bar; leave room for it.
...
Whether the four gaps are acceptable long-term was an owner call, filed as **DEC-260811022536** and **answered 2026-08-11 — partially, by intent.** The two **per-column** gaps (identity/`SERIAL`, `GENERATED`) are closed: they extend the existing column rendering rather than restructure the statement, and they are what an ordinary schema actually hits, since nearly every table has a surrogate key. The two **structural** gaps (inheritance, partitioning) are deferred to **DEC-260811094437**, which carries a named trigger — and that trigger turned out to be more concrete than the one first proposed: it is **not** `Generate Deployment SQL` shipping (that is specified over `db/schema_diff.py` + `db/migration_gen.py`, not over this buffer), but `db/migration_gen.py` gaining table support, which it refuses outright today. At that point it needs a table renderer and `table_ddl.py` is the only pure one in the tree. The buffer's notice therefore names two gaps, not four, and that is a decided boundary pending a consumer rather than an unfinished job.
**Requested:** 2026-08-10
...
**Problem:** The read-only DDL pane (`ui/ddl_editor_panel.py::EditorPanel`) shows a concatenated buffer built by `db/ddl_buffer.py::build_ddl_text(schema)` (verified `ddl_buffer.py:68-120`) that contains **only routines + triggers** (`pg_get_functiondef`/`pg_get_triggerdef`; items list assembled from `schema.routines`/`schema.triggers` at `:78-81`). Tables, views, and matviews have **no DDL shown at all**. Clicking a table node routes to `table_selected` → Properties panel (verified `ddl_buffer_panel.py:786-789`), not to the DDL pane; only span-carrying items (routines/triggers) navigate via `navigate_requested.emit(span.start_line)` (`:781-785`). So a user browsing the schema can read a function's or trigger's source but cannot see a table's/view's definition, and cannot click a table/constraint/FK/index to jump to its DDL. This is a read-only VIEWING/reference gap, not an editing one.

**Proposed approach (grounded, file:line verified):**
- **Extend `db/ddl_buffer.py::build_ddl_text(schema)`** (`:68-120`, currently pure and Qt-free, module docstring `:16-28`) so the ONE concatenated read-only buffer ALSO includes, alongside the existing routines/triggers: **tables** (synthesized CREATE TABLE), **views + matviews** (`pg_get_viewdef` → `TableInfo.view_definition`). Each object gets a banner comment (extend the `_banner` helper `:52-65`) + a `DdlObjectSpan` (`:36-49`), so folding and click-to-line navigation work uniformly for every kind. Owner chose to KEEP the single-concatenated-buffer + click-jumps model over load-on-click.
- **NEW pure, Qt-free CREATE TABLE synthesizer** in `db/` (Postgres has no `pg_get_tabledef`, so it must be built). Owner-confirmed shape: `CREATE TABLE` with columns (name/type/nullable/default from `ColumnInfo`, `introspect.py:46-60`) + **inline** PK/FK/UNIQUE/CHECK constraints (from `ConstraintInfo` + `pg_get_constraintdef`, shipped by FQ-025, `introspect.py:136-171`), plus appended non-constraint `CREATE INDEX` lines (from `IndexInfo`, `introspect.py:173-224`, skipping constraint-backing indexes) and `COMMENT ON TABLE/COLUMN` (from `TableInfo.comment`/`ColumnInfo.comment`). NO ALTERs (owner: "don't worry about ALTER"). A faithful **reconstruction** of the current definition, NOT byte-identical to original creation DDL. **Known reconstruction gaps to flag (out of v1):** SERIAL/identity sequences, GENERATED columns, table inheritance, partitioning — synthesizer covers columns+constraints+indexes+comments only.
- **Views/matviews** use `pg_get_viewdef` — available as `TableInfo.view_definition`, BUT currently fetched only by `snapshot_for_baseline` via `_VIEWDEFS_SQL` (`introspect.py:500-507`), NOT by the DDL Explorer's `fetch_routines_and_triggers`. So the explorer fetch path must be extended to also fetch view defs (and confirm constraints/indexes are on the explorer path — FQ-025 added the index query there; confirm constraints too).
- **Click-to-navigate for ALL tree items** (owner: "on click of any item, take me to the DDL"): extend `BrowserPanel._on_item_clicked` (verified `ddl_buffer_panel.py:781-789`) so table/view/matview nodes — AND newly-added **constraint, FK, and index** nodes — carry a `DdlObjectSpan` and emit `navigate_requested(span.start_line)` (`:386`) → `EditorPanel.navigate_to_line` (`ddl_editor_panel.py:184`). Today only span-bearing routine/trigger leaves do this; table/column rows fall through to `table_selected`/Properties (`:786-796`).
- **"Also constraints, foreign keys" (owner addition) — interpretation, flagged as open question:** add **constraint / FK / index nodes to the tree**, under each table (e.g. a "Constraints"/"Indexes" sub-group mirroring the existing "Columns" group), each **clickable and navigating to its DDL** — for constraints/FKs to their **inline line within that table's CREATE TABLE**; for indexes to their `CREATE INDEX` line. Honors "no ALTERs" (constraints stay inline). Reuse FQ-025's `ConstraintInfo`/`IndexInfo` (already shipped). Exact jump target is an open question below.
- **Read-only / viewing-only, unchanged:** pane stays read-only (`ddl_editor_panel.py:113`); tables/views/constraints are NOT part of the checkout/versioning model (only routines/triggers get `ddl/*.sql`, §18.2). Purely a reference/navigation aid — no edit, no checkout.
- **Size mitigation** (owner accepted the concatenated model): synthesize table DDL **lazily / cache it** (a 300-table schema would otherwise re-synthesize a multi-MB buffer on every reparse), rely on existing per-object folding. Perf on large schemas is a stated design concern.

**Alternatives considered:** *Load the clicked object's DDL on demand* (vs the concatenated buffer) — rejected by the owner in favor of the one-buffer, click-jumps model (matches "take me to the DDL"), with lazy-synthesis/folding as the size mitigation. *Constraints/FKs as separate `ALTER TABLE ADD CONSTRAINT` statements* — rejected: owner said "no ALTERs"; constraints render inline in the CREATE TABLE and their tree nodes jump to that inline line. *CREATE-TABLE-only, no indexes/comments* — rejected by owner (wants the full "this is the table" picture). *Sequences/types/other objects* — owner did NOT choose the wider scope; v1 = tables + views + matviews + existing routines/triggers (+ constraint/FK/index navigable nodes).

**Suggested placement:** EXTEND **§18.1** (DDL Explorer — the read-only buffer + tree + click-navigation, which today documents routines/triggers only). The buffer synthesizer extension + the new CREATE TABLE synthesizer live in `db/` (`ddl_buffer.py` + a NEW pure Qt-free synth module) — this is the one genuinely new module. Cross-reference **§18.7** (change lands in `BrowserPanel`/`EditorPanel`, so both the quality and sandbox instances get it). Reuse FQ-025's constraint/index introspection. **Dependency:** builds on **FQ-025 (PROCESSED)** — its constraint/index introspection widening (`conname` + unique/check + indexes, `_CONSTRAINTS_SQL`/`IndexInfo`) is the raw material the synthesizer + new nodes consume; already shipped, this reuses it, does not re-add it. Not a new section. Note this is distinct from the other queued DDL-Explorer entries: FQ-025 (ALTER ops → editable tab), FQ-024 (context-menu collapse), FQ-260810180336 (name-filter bar), FQ-260810165518 (quality highlight color), FQ-022 (sandbox instance) — none add read-only DDL for tables/views or make constraints/FKs/indexes navigable, so this is a genuinely separate idea, not a dup.

**Open questions (non-blocking, design-time):** (1) constraint/FK/index node click → jump to the **inline constraint line** within CREATE TABLE (recommended) vs a dedicated per-constraint span; (2) does clicking a TABLE node still populate the Properties panel (additive) or only navigate the DDL pane now — recommend additive (both, since `_on_item_clicked` currently emits `table_selected`); (3) column-node click → jump to that column's line within CREATE TABLE, or to the table's banner; (4) object ordering in the buffer (dual-grouped like the tree — tables/views then routines/triggers — vs alphabetical); (5) lazy-synthesis/caching strategy + invalidation on reparse; (6) reconstruction gaps (identity/generated/partitioning/inheritance) acceptable for v1? — note them in the pane (e.g. a comment) or silently omit?

---

## FQ-260811020328: Quality SQL Console — an ad-hoc read/write SQL console targeting the quality (production) database
**Status:** PROCESSED (spec §18.5 D4b; `db/quality_query.py` + `ui/sql_console_panel.py`, commit 6258349; 7585 passed / 51 skipped, 70 new tests)
**Closing note:** shipped under two owner rulings — `DEC-260811023646` (run into an uncommitted transaction, explicit Commit gesture) and `DEC-260811025132` (`Ctrl+Return` kept, conditional on that commit model). The panel was **parameterized, not forked**: a `ConsoleFlavour` record carries the tab key, title, every target-naming sentence, the danger flag and the commit model, so no `if quality:` appears anywhere in the UI.

**Both mechanisms this entry and the spec named turned out to be impossible under the ruling, and neither was a detail.** D4b's `run_quality_query(params: ConnectionParams, …)` can only open and close a connection within one call, so it cannot hold a transaction open between the run and the Commit; and `apply_ddl(..., commit=False)` — cited in the decision entry itself — rolls back **before returning**. The shipped seam is therefore a new `QualitySession` holding the transaction across calls, which also gives the quality lane the same *one object answers which database this can reach* guarantee the sandbox lane has. `db/apply.py` was not touched. §18.5's *"exactly three connection-opening seams"* invariant becomes **four**.

**Four lifecycle edges, not the three the ruling named.** Tab close and window close both **veto** through a dedicated `confirm_discard` seam — kept separate from the object-change dialog because that one carries a *"don't ask again this session"* checkbox, which must never be offered for *"discard your uncommitted production transaction?"*. Connection loss is terminal and states that the server rolled the run back so **nothing** was committed; `close()` closes the handle even when the rollback itself raises, and reports that error rather than claiming a clean discard. The fourth, which nobody had named: **the target stops resolving** — discarded without asking, since the user did not initiate it and there is no database left to commit to. A failed run is rolled back immediately and says so, because PostgreSQL has already aborted the transaction and offering Commit would be a lie.

**The most valuable edit was outside the feature.** `ui/sql_results_panel.py`'s failure block asserted that *"N earlier statements already ran and **COMMITTED** (each statement of a Run is its own transaction)"* — true of the sandbox, false inside one transaction, and worst of all false **on production**. `run_status_lines` gained a transactional branch rather than shipping a sentence this feature falsifies.

Open question the entry left and shipping settled: the danger marking is a **banner strip above the console**, painted with the Explorer's `danger_selection_colors` through a widget-level stylesheet — chosen over tinting the editor because it is unambiguous, does not fight the SQL highlighter, and reads at a glance. Asserted in rendered pixels in both themes, plus a test that no third red entered the app. Still the owner's to reverse. The sterner-confirm-wording question is answered by the flavour record naming the target in every sentence; the inherited **no-Cancel-button** gap is unchanged and still open.
**Requested:** 2026-08-11
**Idea (verbatim/summarized):** Owner: "there's a sandbox sql console. I need the same for quality" — a **Quality SQL Console** that mirrors the existing Sandbox SQL Console exactly but runs against the quality (production-ish) database.

**Problem:** The app has a Sandbox SQL Console (`pgtp_editor/ui/sql_console_panel.py`, opened via `Database ▸ Sandbox SQL Console…`) for ad-hoc SQL against the disposable sandbox, but **no equivalent affordance for the quality database**. Today quality is only reachable read-only (`db/introspect.py::run_queries`) or written through the narrow, explicit "Apply to quality" DDL-object gesture (`db/apply.py::apply_ddl`). The owner wants the full console experience — arbitrary DML/DDL, results grid, completion, formatting — pointed at quality, so a one-off quality query or fix does not require dropping to an external tool like DBeaver (which is exactly the "only option is to break the DB" gap this app exists to close — here reintroduced deliberately, see Reversal below).

**Owner decisions already settled (recorded, NOT to be re-litigated):**
- **Capability = full read/write, mirror the sandbox console exactly.** Arbitrary DML/DDL against quality, not SELECT-only. The owner chose this over a read-only variant with the hazard spelled out.
- **Gate = always available whenever a quality connection (with password) exists.** No Maintenance-mode gate and no extra per-run confirm beyond what the sandbox console already does (the object-change confirmation before DDL/unknown statements). The owner chose this over a Maintenance-only gate and over an always-available-but-confirm-writes middle option.

**Proposed approach:** Add a **second single-instance dynamic center tab**, "Quality SQL Console", opened via a new `Database ▸ Quality SQL Console…` menu action, that reuses the sandbox console's entire UI and editor stack and differs only in its execution target and connection source:
- **Reuse verbatim** (the console is largely target-agnostic already): the `CodeEditor(language="sql")` + `SqlResultsPanel` QSplitter layout, `split_statements()`, the `QueryResult{columns, rows, truncated, row_limit, command_status, error}` result shape, the object-change confirmation dialog before DDL/unknown statements, the 1,000-row default cap adjustable to 100,000, the `run_async` off-GUI-thread seam, `SchemaIndex` completion via injected `set_schema_index(...)`, and results staying in the console's own `SqlResultsPanel` (no `[…]` Audit/Results prefix, not routed to the Audit dock). Ideally the console widget is parameterized by an injected executor rather than forked, so the two consoles are one class with two seams.
- **Net-new engineering — the core of the work:** quality has **no live write session** in the app. `db/sandbox_query.py::run_sandbox_query` accepts ONLY a `SandboxSession` by deliberate construction; there is no "run against quality" path, not even a disabled one. So a **new execution path** is required — e.g. `db/quality_query.py::run_quality_query(params: ConnectionParams, sql, max_rows) -> QueryResult` — that takes quality's `ConnectionParams` (from `db/config.py::connection_from_tree`, which requires a password), opens a write connection, and applies the **same** row cap, 30-second statement timeout, and one-committing-transaction semantics as the sandbox path, returning the identical `QueryResult` shape. The console's `session_provider` seam (today `Callable[[], SandboxSession | None]` from `ui/sandbox_controller.py::SandboxController`) is replaced for the quality instance by a params-provider that supplies the quality `ConnectionParams`, and the executor seam calls `run_quality_query` instead of `run_sandbox_query`.
- **Availability wiring:** the open action is enabled exactly when a quality connection with a password exists (mirror the enablement logic the `Run on quality` leg and the quality-connectivity indicators already use — `active_target_params()` / the §18.8 Quality node), and absent/disabled otherwise, consistent with §26's "absent, not disabled, until reachable" console convention.

**Danger marking (RECOMMENDATION, not owner-settled — flag at design time):** the owner recently made the quality DDL Explorer's selection highlight rose/red to mark it as the dangerous instance (queued as FQ-260810165518). Propose the Quality SQL Console carry the **same rose/red danger marking** (e.g. on the editor and/or results, reusing the palette-override precedent at `ui/sql_results_panel.py:545-550`) plus a distinct tab title ("Quality SQL Console"), so it can never be visually confused with the sandbox console. This is a proposal; the owner has not ruled on it.

**Keyboard (mirror the sandbox console exactly — no new chords invented):** verified against `docs/KEYBINDINGS.md`:
- **Run = `Ctrl+Return`**, hosted **on the console tab** as a `QShortcut` with `WidgetWithChildrenShortcut`, calling the same `run()` the Run button calls — identical to the sandbox console's row. Gate: **DEC-009** (no command form — a button is not a command form; the sandbox row is explicitly gated DEC-009 for this reason, and reserved `yes`). The quality console gives the identical answer. Under DEC-014, `sql_console_panel.py` is already one of the six editing surfaces stating its answer for every reserved editor chord; a second instance of the same panel class inherits those answers unchanged — **no new `RESERVED_SEQUENCES` entry and no ledger row is needed**, because it is the same host class on the same chords. **Caveat to confirm at implementation:** if both consoles can be open at once (see open questions), `Ctrl+Return` is a `WidgetWithChildrenShortcut` scoped to each tab's own widget subtree, so the two instances do not collide (each fires only when its own tab has focus) — verify this holds and does not create two enabled shortcuts in one focus chain.
- **Open action `Database ▸ Quality SQL Console…`** mirrors the sandbox open action: a menu `QAction` that (like the sandbox open action) carries **no shortcut** — the sandbox open action has no row in `docs/KEYBINDINGS.md`, i.e. no chord. Give the quality open action the same treatment (no chord). If a chord is ever wanted here, that is a separate owner decision, not something to invent in this entry.

**Reversal — must be flagged for `spec-maintainer`:** this feature **reverses `CONSOLIDATED_SPEC.md` §18.5 D4** ("Ad-hoc SQL execution against the sandbox: the Sandbox SQL Console", settled 2026-08-06), whose hard principle reads *"Sandbox-only. May never target production — not behind confirmation, not behind preference."* Whoever picks this up MUST dispatch `spec-maintainer` to fold it with a **Supersession Ledger row** overriding that clause, and note in the spec that this reintroduces an unguarded production-write channel — accepted deliberately by the owner, against the app's safe-IDE identity, with the two owner decisions above as the justification.

**Alternatives considered:**
- *Read-only (SELECT-only) quality console* — **rejected by the owner** in favor of full read/write, hazard acknowledged.
- *Maintenance-mode gate / confirm-before-every-write* — **rejected by the owner** in favor of "always available whenever a quality connection with password exists", no extra confirm beyond the existing object-change dialog.
- *Point the existing sandbox console at quality via a target toggle* — rejected on construction grounds: `run_sandbox_query` takes only a `SandboxSession` and there is deliberately no target affordance; a toggle would blur the sandbox/quality boundary the danger-marking exists to keep sharp. A **separate, distinctly-marked second tab** is safer and matches the DDL-Explorer precedent (two role-scoped instances, quality one marked red).
- *Brand-new spec section* — rejected; this is a variant of the same feature and belongs beside the sandbox console under §18.5 D4 so the "may never target production" clause is reconciled in place rather than left contradicting a new section.

**Suggested placement:** **EXTEND §18.5 D4** (the SQL-console section) — add the quality variant beside the sandbox one and **reconcile/supersede** the "may never target production" clause with a Supersession Ledger row. Reuse map: `CodeEditor`, `SqlResultsPanel`, `split_statements()`, the `QueryResult` shape, the object-change confirm, dynamic single-instance-tab hosting, `SchemaIndex` completion (`set_schema_index`), and the `run_async` off-thread seam are all reused; only the **execution path** (new `run_quality_query` taking `ConnectionParams` vs `run_sandbox_query` taking `SandboxSession`) and the **connection source** (`connection_from_tree` / `active_target_params()` vs `SandboxController`) differ. Cross-reference §26 (console availability convention) and §18.8 (Quality node, for the enablement gate). Distinct from FQ-020's `Run on quality` (a DDL-object deploy leg, not an ad-hoc console) and from FQ-260810165518 (DDL Explorer highlight, not the console) — this is a genuinely separate feature, not a dup.

**Open questions (for whoever designs/implements — NOT answered here):**
- **Multi-statement transaction semantics against quality.** The sandbox console runs all statements in one committing transaction; the "mirror exactly" directive implies the quality console does the same. But this is a **real safety fork the owner has not ruled on** for production: should quality default to per-statement commit, or require an explicit confirm-before-commit, instead of one all-or-nothing committing transaction? Flag to the owner before shipping the commit behavior.
- **Inherited gaps carry over.** The sandbox console has a hardcoded **30-second statement timeout and no Cancel button** (an acknowledged v1 gap). These carry into the quality console unchanged unless the owner wants them addressed — arguably more pressing against production, but not settled.
- **Object-change confirm wording.** Should the confirmation dialog before DDL/unknown statements be **sterner for quality** than for sandbox (e.g. name the quality database explicitly), or use the identical text? Recommend at least naming the target; owner to confirm.
- **One-instance-per-target.** Is there one Quality console tab and one Sandbox console tab (two distinct single-instance tabs), and can both be open simultaneously? "Mirror exactly" implies yes — one of each, both openable at once — but confirm, since it bears on the `Ctrl+Return` focus-scope caveat above.
- **Danger marking (see Recommendation).** Rose/red marking + distinct title is proposed, not owner-settled.
...
**Problem:** FQ-032 shipped `v`/`V` as insert-entry aliases with **no side effect** — they drop to Edit mode and the user selects the Windows-native way — and the Command-mode operators `d`/`c`/`y`/`x` are **motion-only**: they cannot act on a pre-existing selection. The result is that a vim user's most ingrained reflex, "`v` → move to extend → `d`/`y`/`c`", has no analogue at all: there is no way to visually grow a selection by keystroke and then operate on it. FQ-032 deliberately declined a full Visual mode (selection is "a Windows method"), which is correct for the mode model but leaves the extend-then-operate gesture entirely unserved. The owner wants that gesture back at a cost of one extra `Esc`, without re-introducing a vim Visual sub-mode or vim registers.

**Proposed approach (owner-final middle-ground — do NOT re-challenge the middle-ground choice):**
- **Two NEW Edit-mode selection primitives, each a menu command with NO Edit-mode chord:**
  1. **Sticky selection** (character-wise): a toggleable Edit-mode state in which ordinary cursor movement EXTENDS the selection, as if `Shift` were held down continuously.
  2. **Line selection** (line-wise): the same, but movement selects WHOLE LINES (vim `V` feel).
- **`v` / `V` stay insert-entry aliases** — they still return to Edit mode and there is STILL no vim Visual sub-mode, so they stay in `INSERT_ENTRY_ACTIONS` (`pgtp_editor/vim/grammar.py:88`). They now ALSO switch on sticky selection (`v`) / line selection (`V`) respectively, by invoking the **SAME underlying toggle the menu action invokes** — one code path, not a parallel one, so the sticky-state cannot drift between the two entry points (this mirrors FQ-032's `:` palette calling `QAction.trigger()`).
- **Selection is preserved across the Edit↔Command boundary** exactly as it is today — FQ-032 already keeps selection across the mode switch; keep that unchanged.
- **Command-mode operators `y`/`c`/`d`/`x` become selection-aware:** when an existing selection (character or line) is present, they act on it instead of requiring a motion. `c` deletes the selection and drops to Edit mode; `d`/`x` delete it; `y` copies it. They write the single system clipboard — the **unchanged FQ-032 clipboard model**: one shared system clipboard, NO registers, paste stays inline/charwise.
- **Resulting muscle-memory flow and its ACCEPTED, STRUCTURAL cost (recorded explicitly):** `v` → move to extend → **`Esc`** → `d`/`y`/`c`/`x`. This is ONE extra `Esc` versus real vim's `v` → move → `d`. The owner explicitly accepted this ("this only adds an `Esc` between vim and pgtp editor"). The extra `Esc` is **structural** — operators exist only in Command mode, so you must step back into Command mode before operating. **"Fixing" this by adding a true Visual mode is explicitly OUT OF SCOPE and was rejected by the owner.**

**Where it wires (verified this session):**
- **Grammar (`pgtp_editor/vim/grammar.py`, pure/Qt-free, §8):** `v`/`V` stay in `INSERT_ENTRY_ACTIONS` (`:88`). The Qt-free grammar itself is largely unchanged — the selection state and the toggle live on the editor (Qt) side, not in the grammar. What the grammar's DOCSTRING asserts must be revised, not the parsing: `:36-37` ("**No visual mode.** `v`/`V` are insert-entry aliases … there is no selection state in this machine either") and the operator contract at `:46` documented as "**MOTION-ONLY** (no visual-selection target, since there is no visual mode)". Both are what this feature revises — `v`/`V` keep being insert-entry but gain a selection-toggle side effect, and operators become selection-aware. (Note: `v`/`V` staying insert-entry means "no visual **MODE**" is technically still true; the precise thing being reversed is that they had **no side effect** and that operators **ignored selections**.)
- **Editor/Qt side (`pgtp_editor/ui/vim_mode.py::VimModeMixin`, the shared mixin on `CodeEditor` and `XmlEditor`, active on all six editable surfaces):** this is where the sticky/line-selection STATE must live and where selection-aware operators must be wired. Operator execution is `_vim_run_operator` (`:966`), dispatched from `_vim_run` (`:733`); insert-entry actions run through `_vim_run_action` (`:1069`). The Edit-mode movement path that must extend-with-anchor when sticky is on is the same key handling `handle_command_mode_key` / the editor's Edit-mode `keyPressEvent` feed. Put the toggle and the sticky-state on the shared mixin (or `ui/editor_shared.py` for anything cross-family), NOT duplicated per editor — FQ-032 established `ui/editor_shared.py` as the one home for cross-editor behavior.
- **Mode indicator (`pgtp_editor/ui/mode_indicator.py`):** this design adds NO new editing-mode label — there is no Visual mode, so `EDITING_EDIT`/`EDITING_COMMAND` (`:134-135`) and `mode_text(major, minor, editing)` (`:165`) stay as-is. BUT sticky/line-selection being ON is a real state the user needs to see (otherwise "why is my cursor selecting?"). See Open questions — a subtle suffix on the Edit-mode segment (e.g. "Edit mode · Sticky" / "· Line") is the recommendation; owner to confirm whether and how.

**Keyboard gate (owner-answered; recorded, verified against `docs/KEYBINDINGS.md`):**
- The two toggles (sticky selection, line selection) are **command forms with a menu entry and NO Edit-mode chord.** Under **DEC-012** a command with a command form gets exactly ONE keyboard host, the `QAction` — here that `QAction` carries **NO `setShortcut`**. Recorded.
- **`v` / `V` are NOT a second host.** They are the vim grammar's Command-mode keys, already handled inside the vim interceptor (per DEC-014, the editing surfaces state their answers; `v`/`V` are grammar tokens, not registered chords). **`v`/`V` do not appear as bound chords in `docs/KEYBINDINGS.md`** (confirmed — the ledger's rows are the modifier chords the app binds; `v`/`V` are answered inside Command-mode key handling, not via `setShortcut`/`QShortcut`). They now additionally invoke the toggle by triggering the SAME action/code path, so there is **no new chord and no ambiguous binding.**
- **This change adds NO new chord.** The MEANING change to `v`/`V` (insert-entry → insert-entry + selection-toggle) is a documentation/grammar annotation, not a new binding — nothing in `docs/KEYBINDINGS.md` needs a new row, and no `RESERVED_SEQUENCES` change is implied. If implementation finds anything that contradicts "no new chord," raise it as an open question rather than inventing a binding.

**Alternatives considered:**
- **A true vim Visual sub-mode** (`v`/`V` enter a real VISUAL mode with its own operator grammar) — **REJECTED by the owner** as the deliberate middle-ground decision. It would reverse FQ-032's "no visual mode" model wholesale, re-introduce a mode the mode indicator would have to name, and duplicate the Windows-native selection FQ-032 chose on purpose. The middle-ground keeps Edit mode as the selection surface (Windows-native movement + a sticky toggle) and reuses the existing operators, at the cost of one `Esc`. **This choice is settled; not re-litigated here.**
- **Add `v`/`V` as no-op insert-entry aliases only** (status quo) — rejected: leaves the extend-then-operate gesture completely unserved, which is the whole point of the request.
- **A parallel sticky-selection code path invoked by `v`/`V`, separate from the menu action** — rejected on drift grounds: two paths toggling one state can disagree. The owner's design mandates ONE toggle invoked by both the menu `QAction` and the `v`/`V` keys.

**Suggested placement:** **EXTEND §8** (the shared editor infrastructure + the FQ-032 vim/editing-mode layer). Add the sticky-selection and line-selection Edit-mode primitives (with their menu entries), wire `v`/`V` to invoke the same toggle, and extend the Command-mode operators `y`/`c`/`d`/`x` to act on an existing selection. Reuse `ui/editor_shared.py` for any cross-family behavior and the `VimModeMixin` for the sticky-state and selection-aware operators — do NOT duplicate logic per editor. **This REVERSES two shipped §8/FQ-032 assertions** — "no visual mode / `v`,`V` are pure insert-entry aliases with no side effect" and "operators are motion-only" — so whoever implements it MUST dispatch `spec-maintainer` to fold it with a **Supersession Ledger row against §8**, not fold it silently. (Precise scope of the reversal: `v`/`V` gain a selection-toggle side effect while staying insert-entry, and operators gain a selection target; "no visual **mode**" remains literally true.) Cross-reference FQ-034 (structural expand-selection): that is a DIFFERENT gesture — grows a selection through nested SQL structure via `Ctrl+Shift+A` — and is orthogonal to character/line sticky selection here; cross-ref, do not conflate. Sticky/line selection is an Edit-mode feature and should work on all six editable surfaces via the shared infrastructure, exactly as FQ-032's mode layer does.

**Open questions (list; owner to answer where noted — recommended defaults given, NOT owner-settled):**
- **Sticky-selection exit conditions** (defines the feel): when does it turn OFF? Recommend: explicit menu toggle-off; typing a printable character replaces the selection and exits sticky (resumes ordinary typing); a Command-mode operator consuming the selection collapses/exits it; a plain mouse click (no drag) exits. Owner to confirm.
- Are sticky (char) and line selection **mutually exclusive** (turning one on turns the other off — two granularities of one "select mode")? Recommend yes.
- Does `v`/`V` (or the toggle) **restart the anchor at the caret** if a selection already exists, or extend the current one? Recommend fresh anchor at caret.
- **Line-selection semantics:** include the caret's starting line and extend whole lines in both directions (vim `V` feel)? Recommend yes.
- **Paste stays inline/charwise** (no linewise register), so `V`→`Esc`→`y`→`p` pastes INLINE, not as a new line — consistent with FQ-032's single-system-clipboard model. Noted here so it is not mistaken for a bug.
- **Menu location and labels** for the two toggles — ground the actual menu tree at design time (likely an Edit/Selection area, e.g. beside the `Select` menu FQ-015/FQ-034 established) and propose a home; do not guess the exact path in advance.
- **Mode-indicator surfacing of sticky/line ON** (see grounding note): whether/how to show it. Recommend a subtle suffix on the Edit-mode segment ("Edit mode · Sticky" / "· Line"); owner to confirm.
...
**Problem:** The app's configuration surfaces are scattered across three menus and cannot be reached from the launcher, which maps one button to one command. Today: `Settings ▸ Edit Snippets…` and `Settings ▸ Autoformatter settings…` (the Maintenance-only `Settings` menu, FQ-027/DEC-006), plus `View ▸ Customize Toolbar…` and `View ▸ Customize Shortcuts…` (reachable any time). There is no single "here are the app's settings" entry point, and the launcher's Maintenance group cannot offer settings because a launcher button derives from one menu command, not a whole menu. The owner wants one cohesive settings home reachable as a single launcher button and a single menu entry.

**Proposed approach (owner-settled two-pane shape — do NOT re-challenge the consolidation choice):**
- **One two-pane dialog:** left = a list/tree of the six setting categories; right = the selected category's settings pane. The six panes, four reusing existing surfaces and two unimplemented placeholders:
  1. **Snippet settings** — REUSE the existing `pgtp_editor/ui/edit_snippets_dialog.py` widget + `ui/snippet_controller.py::SnippetController` as a pane (today `Settings ▸ Edit Snippets…`, command id `settings.edit-snippets`, built in `ui/main_window.py::_build_settings_menu` ~:3874; FQ-030).
  2. **Customize Toolbar** — RELOCATE the existing `ui/toolbar_controller.py` customize surface into the dialog as a pane (today `View ▸ Customize Toolbar…`, `ui/main_window.py:3490`, `_toolbar_ui.open_customize_dialog()`; FQ-004).
  3. **SQL/plpgsql formatter settings** — REUSE the existing `pgtp_editor/ui/autoformat_settings_dialog.py` (`MENU_LABEL = "Autoformatter settings…"`, `open_autoformat_settings`) as a pane (today `Settings ▸ Autoformatter settings…`; FQ-033, §18.4).
  4. **Keyboard shortcuts** — RELOCATE the existing `ui/customize_shortcuts_dialog.py` into the dialog as a pane (today `View ▸ Customize Shortcuts…`, `ui/main_window.py:3498`, `open_customize_shortcuts_dialog()`; FQ-012). **Note it is a NON-MODAL dialog today** — embedding it as a pane inside a settings dialog changes that (see Open questions).
  5. **Syntax highlight colors** — NOT IMPLEMENTED. Placeholder pane; see child entry **FQ-260812002828**.
  6. **Color scheme** — NOT IMPLEMENTED beyond the binary `View ▸ Light Theme` toggle (`ui/main_window.py:3484`, `_light_theme_action`, checkable). Placeholder pane; see child entry **FQ-260812002829**.
- **Reuse the existing dialog widgets as embedded panes — do NOT reimplement them.** The four working panes are the existing controllers/dialogs re-hosted, so their behaviour and tests stay intact; only their host (a standalone dialog vs. an embedded pane) changes.
- **RELOCATE means MOVE — the four absorbed surfaces are REMOVED from their current locations (owner-settled 2026-08-12: *"relocating means moving, so they won't be anymore where they were before."*).** The consolidated Software Settings dialog becomes their SOLE entry point. Specifically: `View ▸ Customize Toolbar…` is removed from `View`; `View ▸ Customize Shortcuts…` is removed from `View`; `Settings ▸ Edit Snippets…` is removed as a standalone `Settings` entry; `Settings ▸ Autoformatter settings…` is removed as a standalone `Settings` entry. The `Settings` menu ends up with a **single `Software settings…` entry** (the consolidated dialog), and `View` loses both of its customize entries. This is owner-settled, NOT an open question.
- **Surfacing (single command, two entry points):**
  - A NEW single command — proposed label `Software settings…`, proposed command id e.g. `settings.software` (verify the id is unused at design time). It opens the two-pane dialog.
  - **Launcher button:** add the command id to the **Maintenance** tuple in `pgtp_editor/ui/launcher_dialog.py` — today `("schema.edit-xsd", "schema.import-xsd")` at `:158` (verified this session). The launcher derives the button label from a menu walk, so the command MUST exist as a real menu action or no button label is produced.
  - **Settings-menu entry:** a single `Settings`-menu entry for the same command. The `Settings` menu is Maintenance-only per `_MAINTENANCE_ONLY_MENU_TITLES = ("Settings",)` (`ui/main_window.py:353`, FQ-027/DEC-006).
- **Rationale (record this):** the single-command design is exactly what resolves the "a launcher button maps to ONE command but Settings is a whole menu" mismatch — one command hosting a multi-pane dialog is both a valid launcher button and a valid single menu entry.

**Keyboard gate (verified against `docs/KEYBINDINGS.md` this session):**
- The new `Software settings…` command HAS a command form (a menu-bar entry AND a launcher button) → under **DEC-012** it gets **exactly ONE keyboard host, the `QAction`**. Per the Maintenance-only `Settings`-menu convention (FQ-027/DEC-006), it takes **NO shortcut** — the `QAction` carries no `setShortcut`. **This change adds NO new chord**; I read the full ledger and confirmed nothing needs a new row and no `RESERVED_SEQUENCES` change is implied.
- The embedded panes' own internal buttons/fields stay inside the dialog and are not window chords. **However**, folding `Customize Shortcuts…` (FQ-012) inside changes WHERE that pane lives, not what it does; its own internal behaviour (listing commands, rebinding) is unaffected by this entry — but its host-modality changes (Open questions).

**Alternatives considered:**
- **Two more launcher buttons (Edit Snippets, Autoformatter settings) + leave the View items where they are** — REJECTED by the owner in favour of one consolidated surface. It would grow the Maintenance launcher column ad hoc, still leave toolbar/shortcut customization out of any "settings" home, and give the user six scattered entry points instead of one.
- **A `Settings` submenu surfaced as a launcher fly-out** — rejected: the launcher's contract is one button = one command (a menu walk of a single id), and a fly-out would break that uniform shape (`LAUNCHER_GROUPS` is `(title, (command-ids…))`). The consolidated dialog keeps the launcher contract intact.
- **A settings dialog that reimplements each pane fresh** — rejected on cost and drift: the four working surfaces already have widgets, controllers and tests; re-hosting them as panes preserves all of that. Reuse, don't rebuild.

**Suggested placement:** **CREATE a new consolidated "Software settings" section**, which **REORGANIZES and supersedes the scattered placement** of several shipped surfaces. It must cross-reference and reconcile: §7/FQ-027 (the Maintenance-only `Settings` menu), FQ-011 (the launcher menu-filter), FQ-027 (the three-column launcher), FQ-012 (Customize Shortcuts), FQ-004 (Customize Toolbar), FQ-030 (Edit Snippets), and FQ-033/§18.4 (Autoformatter settings). Because it MOVES/relocates the placement of those settings surfaces, whoever implements it **MUST dispatch `spec-maintainer` with Supersession Ledger rows** for each relocated surface — do not fold silently. Reuse (do not reimplement) the existing dialog widgets as panes; reuse the launcher's `(title, ids)` tuple contract and the Maintenance-only menu-gating mechanism rather than inventing new plumbing. The two placeholder panes are tracked as child entries **FQ-260812002828** (syntax highlight colors) and **FQ-260812002829** (color scheme).

**Open questions (owner to answer at design time — flagged, not assumed):**
- **Maintenance-only trade — HEADLINE, confirm-the-trade (settled in direction, one thing left to confirm).** The relocation is owner-settled as a MOVE (above): Customize Toolbar and Customize Shortcuts, reachable ANY time today via `View`, move into the Maintenance-only Software Settings dialog and are therefore **reachable only in Maintenance mode**. This is CONSISTENT with FQ-027's existing design that the app is configured in Maintenance mode — the `Settings` menu is already Maintenance-only for exactly this reason — so it is likely intended. **But it does remove the ability to rebind a shortcut or customize the toolbar outside Maintenance mode**, so the owner should confirm that trade explicitly at design time. This is a confirm, not a re-open of the MOVE decision.
- **Modality of the embedded shortcuts pane.** Customize Shortcuts is NON-MODAL today; embedding it as a pane in a (likely modal or single-window) settings dialog changes that. Confirm the settings dialog's modality and whether the shortcuts pane's live-preview/non-modal behaviour must be preserved.
- **Placeholder panes' presentation.** Should the two unimplemented panes (FQ-260812002828, FQ-260812002829) appear in the left list as **present-but-disabled / "coming soon"**, or be **omitted entirely** until their owner descriptions land? Owner to decide; this governs whether the dialog ships showing six categories or four.
...
**Problem:** There is today NO user-facing customization of editor syntax-highlight colors. Highlight colors are hardcoded — e.g. `pgtp_editor/ui/xml_editor.py` (`set_colors`), the SQL/code highlighter, and the palette in `pgtp_editor/ui/theme.py` (the Highlight palette). The owner wants a Software Settings pane to let the user customize these colors, but the design is not yet specified.

**Proposed approach:** UNSPECIFIED — reserved. This is one pane of the FQ-260812002827 two-pane dialog. **Needs owner description before design.** Nothing here is settled: which tokens/languages are customizable, the default palette, how it interacts with the light/dark themes (`theme.py`), persistence (presumably QSettings), and a reset-to-defaults path are ALL open. Do not guess and do not implement until the owner supplies a description.

**Alternatives considered:** None yet — this is a reservation, not a triaged design. Alternatives will be weighed once the owner supplies a description.

**Suggested placement:** A pane WITHIN the FQ-260812002827 "Software settings" consolidated dialog. Whether it lives as a §-level feature or an extension of the highlighting sections (around `ui/xml_editor.py` / `ui/theme.py`) is deferred to the owner-description pass. Do NOT fold into the spec while blocked.

**Open questions (all open — this is why it is blocked):** Which tokens/languages are customizable? What are the default colors? How does per-token color interact with the light/dark theme switch (one palette per theme, or theme-independent)? Where is it persisted (QSettings)? Is there a reset-to-defaults? Should the parent dialog show this pane disabled/"coming soon" or omit it until this is answered (also flagged on FQ-260812002827)? **All require an owner description before this entry is unblocked.**
...
**Problem:** Today the app has only a binary `View ▸ Light Theme` toggle (`pgtp_editor/ui/main_window.py:3484`, `_light_theme_action`, checkable; `ui/theme.py::apply_theme(app, light)`). There is NO color-scheme selection or customization beyond light/dark. The owner wants a Software Settings "color scheme" pane, but "color scheme" as a richer concept — named schemes, custom palettes, editor-vs-chrome separation — is undefined.

**Proposed approach:** UNSPECIFIED — reserved. This is one pane of the FQ-260812002827 two-pane dialog. **Needs owner description before design.** A minimal version could simply relocate the existing binary light/dark toggle into the pane (the existing `_light_theme_action` / `apply_theme(app, light)` could back it), but anything richer than that binary toggle — named schemes, custom palettes, separate editor vs. chrome theming — has no design and needs the owner to describe it. Do not guess and do not implement until the owner supplies a description.

**Alternatives considered:** None yet — this is a reservation, not a triaged design. (The one thing that IS available today, and is noted only as context, is the existing binary light/dark toggle that a minimal pane could relocate; whether that is the whole pane or just its floor is an owner call.)
...
- **Active-theme SELECTION availability — RESOLVED (owner, 2026-08-12): NO always-available switcher.** The binary `View ▸ Light Theme` toggle (`ui/main_window.py:3484`, `_on_light_theme_toggled` `:2040`) is **REMOVED entirely**; theme selection lives ONLY in the Maintenance-mode Themes pane (Feature B, FQ-260812021716). Removing the toggle is part of THIS feature — the theme system replaces it. **Principle the owner stated and that governs this whole area: settings changed in Maintenance mode are APP-WIDE and PERSISTENT.** The mode is session-only, but the configuration it changes is durable, so a chosen theme is saved app-wide the moment it is marked selected and is remembered across app restart AND new session. You set a theme once as a durable preference, not per session — which is exactly why the session round-trip into Maintenance is an acceptable cost, and why no always-available View switcher is needed. Consistent with the same Maintenance-only trade already accepted for Customize Shortcuts/Toolbar (FQ-260812002827).
...
- **Persistence — RESOLVED (owner, 2026-08-12): the selected theme is saved to QSettings BY NAME, app-wide, the moment it is marked selected in the Themes pane, and re-applied on every app start and new session.** This replaces today's persisted boolean `light=true/false` key; migrate an existing stored boolean to the matching bundled theme file (`light=true` → the light theme file, `false` → the dark one). This is the mechanism behind the removed-toggle decision above.
...
**Status:** PROCESSED **EXCEPT PART 4** — the buffer half shipped (`db/pg_dump_mode.py` + `db/pg_dump_ddl.py` + `db/ddl_buffer.py`, commits a852563 / 0025113; wired in 07b83e9 and e6ab9e1). **Part 4, *the sandbox renderer*, never shipped and is tracked as `BUG-260813025054`.** This entry was flipped to PROCESSED on the strength of the buffer work; a wiring audit then found Part 4 was a full section of the body — `pg_dump --schema-only` → `pg_restore` for provisioning, with `build_baseline_sql` kept as the degrade path — not merely a phrase in the title. Sandbox provisioning still calls `build_baseline_sql` unconditionally, which by its own docstring omits primary keys, foreign keys, DEFAULTs, indexes, sequences and extensions. Two further gaps found with it: the **sandbox Explorer never probes its own mode** (`BUG-260813025053`) and the **degrade row has no role gate** (`BUG-260813023714`).
**Closing note:** the first triage called `pg_dump`'s non-contiguity **fatal** to click-to-navigate, then **retracted it** — and the retraction is what made this buildable. In a *whole-database* `--schema-only` dump each `CREATE TABLE`, each `ALTER TABLE ONLY … ADD CONSTRAINT` and each `CREATE INDEX` is **individually** contiguous, so spans are recovered by parsing statement boundaries onto the **existing** `DdlObjectSpan` — no sibling type, §18.1's trap 1 intact. That also killed the `-t` route and its ~300 subprocesses per refresh, now recorded as a trap.

**Two things no entry anticipated.** `pg_dump` splits a table's constraints across **two** shapes — `CHECK` stays inline in the `CREATE TABLE` while PK/UNIQUE/FK come out as standalone `ALTER TABLE` — so handling only the `ALTER` shape would have left every CHECK constraint's tree row clicking into nothing. And **routines and triggers still come from the catalog in full mode**, the dump's own `CREATE FUNCTION` statements omitted rather than shown twice: a routine's identity is `schema.name(format_type(proargtypes))`, and recovering it from a header means *re-rendering* that string with typmods spelled differently on the two sides — which BUG-018 forbids, and a near-miss silently unnavigates a routine and breaks its `Edit DDL`.

Determinism came out **stronger** than specified: end-to-end byte-identity still cannot be promised, because `pg_dump`'s text varies with the client version, and that is written down as an environmental assumption rather than claimed — but composition orders relations by name and attachments by `(kind, name)`, removing `pg_dump`'s walk order from the buffer's identity.

Wiring it established two facts the app had never had: **there was no quality capability probe anywhere** (`db/sandbox.py::probe` had only ever run against the sandbox), and the owner's **same-version principle was assumed and never enforced** — no comparison of the two servers existed until `server_major_divergence`.

Owner rulings honoured as given: the mode is reported on **every** DDL open (a degrade is a *second* row, never a replacement); restricted mode stays offerable as a clone source **with the warning**; and the `SERIAL` sequence hazard is **warn-only**, so no `CREATE SEQUENCE` span is emitted — asserted absent.
...
Arrived as the owner's answer to **`DEC-260811094437`** (do the synthesized `CREATE TABLE`'s inheritance/partitioning gaps ever close). The owner declined all three options and redirected the approach. That decision entry is superseded-in-direction and points here.

**⚖ OWNER RULING (2026-08-12) — SETTLED, verbatim. This supersedes the first triage pass's "separate command" recommendation; do not re-litigate any of it.**

> *"let's make it different. If pg_dump is present and version is correct, the DDL should be full view. If pg_dump is not present (in the path, or at all, or it's not the correct version), it should use the internal restricted ddl. This information should go to the Messages bar **every time DDL is opened** (local pg_dump version mismatches server version). This way the choice is clear. pg_dump presence and version control should run when connected to quality server."*

Plus, on the sandbox renderer: **"Keep it in the pg_dump feature"** — `db/sandbox.py::build_baseline_sql` is **bundled into this entry deliberately**, not filed separately.

What that settles, and a later pass must not "improve" any of it:
- **DUAL-MODE, one buffer.** `pg_dump` present **and** version-correct ⇒ the DDL buffer **is** the full `pg_dump` view. Otherwise ⇒ today's synthesized restricted DDL. **Not** a replacement (the fallback answers the frozen-build objection: a packaged install without client tools simply gets the synthesizer) and **not** a separate command.
- **The user is told EVERY TIME DDL is opened** — not once per session, not only on failure. Owner's stated reason: *"this way the choice is clear"* — the mode must be visible at the moment it matters, and **the repetition is accepted, on purpose**.
- **The presence/version probe runs on CONNECTING TO THE QUALITY SERVER**, not lazily at first DDL open. So the every-open notice **reports the cached probe result**; it does not re-run `pg_dump --version` per open.

**Problem:** `pgtp_editor/db/table_ddl.py` synthesizes `CREATE TABLE` from `pg_catalog` and, by the owner's own ruling `DEC-260811022536`, still does not cover **table inheritance** or **partitioning** — every table's text carries `RECONSTRUCTION_NOTICE` / `RECONSTRUCTION_NOTICE_DETAIL` saying so. It will also never cover exclusion-constraint subtleties, collations, storage parameters, tablespaces, or any future PostgreSQL feature, because it is a hand-written reconstruction chasing a moving target. `pg_dump` is the reference implementation and gets all of that right, for free, forever. **That argument is correct and this entry does not dispute it.** What this entry disputes is the *scope* of "just use pg_dump" and the claim that it "would simplify all ddl issues".

**The crux — verified. ⚠ Read the four obstacles below as the INVESTIGATION, not as the conclusion: obstacles 1 and 2 are ANSWERED in "Proposed approach" Part 3 (statement-boundary parsing on the existing span type, one whole-database dump per build), and 3 and 4 are accepted costs recorded there. Nothing here is a reason to reopen the ruling; it is the reasoning a later reader will otherwise re-derive.**

§18.1's buffer is a **navigable structure**, not text. `table_ddl.RelationDdl` returns `column_offsets` / `constraint_offsets` / `index_offsets`; `db/ddl_buffer.py::_append_relations` turns them into `DdlObjectSpan`s; `ui/ddl_buffer_panel.py` → `ui/ddl_editor_panel.py::navigate_to_line` is the whole point of the Explorer. Four verified obstacles, in order of severity:

1. **`pg_dump`'s output shape structurally contradicts the buffer's span model.** A whole-database `pg_dump --schema-only` is **dependency-ordered globally, not grouped per object**: constraints come out as separate `ALTER TABLE ONLY … ADD CONSTRAINT` statements in a later section of the file; a `SERIAL` column comes out as `CREATE SEQUENCE` + `ALTER SEQUENCE … OWNED BY` + `ALTER TABLE … ALTER COLUMN … SET DEFAULT nextval(…)`, each somewhere else; plus `ALTER TABLE … OWNER TO`, `GRANT`, and a `SET` preamble. **A table's DDL is therefore non-contiguous.** `DdlObjectSpan` carries exactly one `start_line`/`end_line` pair, and `ddl_editor_panel.py::_fold_regions_for_spans` folds **one contiguous region per span**. Non-contiguous objects do not need "a parser" — they need a **different span type**, which is precisely trap 1 that §18.1 forbids ("one dataclass, one `kind` field; a sibling span type forks two walks and the second one written misses a case"). It also contradicts the owner's own recorded scope for this buffer: *"don't worry about ALTER"*, with constraints rendered **inline** for that reason. `pg_dump` is an all-`ALTER` renderer for exactly the things the owner asked to see inline.
2. **Per-table `pg_dump --schema-only -t schema.table` restores contiguity but breaks two other rules.** It costs **one subprocess + one server connection per table** on every Explorer build — and §18.1 open question 5 settled **NO CACHE** ("the panel holds none, matching the app's recompute-fresh posture"), so a 300-table schema means 300 process spawns per refresh, on a code path today served by one psycopg round trip. Worse, `-t` does **not** dump objects the selected table depends on, so the owned sequence behind the very `SERIAL` column the owner opened with may be **missing** from a per-table dump. *(Verify against a real server before designing — this is the single most important experiment in the entry.)*
3. **Determinism is currently a proven property and would become an environmental one.** BUG-018's rule is that the buffer is byte-identical across fetches, pinned by a test that permutes catalog order — possible only because `table_ddl.py` is a pure function. `pg_dump` output is deterministic for a fixed (server version, pg_dump version) pair, but embeds `-- Dumped from database version …` / `-- Dumped by pg_dump version …` and a version-dependent `SET` preamble, so it is **not** stable across a client upgrade, and **not** identical between the two §18.7 Explorer targets when quality and sandbox run different majors. Whatever the tests then pin is our parser against canned text, not the renderer.
4. **"pg_dump is correct so we needn't be" only holds if we never parse it.** The moment spans require parsing, the project owns a dependency on `pg_dump`'s **text layout**, which is not a documented contract — whereas `pg_catalog` is documented and versioned. That inverts the reliability argument that motivates the idea.

**Facts on the other named questions:**

- **Version matching.** `pg_dump` dumps from older servers fine and **refuses a server newer than itself** (`aborting because of server version mismatch`). Detection is nearly free and half-built: `db/sandbox.py::probe` already decodes `server_version` from `server_version_num` **and** already resolves `pg_dump_path` via the injectable `which` seam; `pg_dump --version` supplies the other half. The failure must be a **named refusal** in the shape of `MissingCloneToolError` — `clone_data` sets the precedent explicitly ("never silently falls back to schema-only").
- **Dependency kind-change — this is a real regression risk.** Today `pg_dump` is **optional and degrades**: `sandbox.py`'s docstring calls D2a "a deliberate, narrowly-scoped exception … applies ONLY to the with-data clone path", and the schema-only path "never requires `psql`/`pg_restore`". Making the Explorer require it converts a **shipped, working, read-only feature** into an unavailable one, in a §18 architecture built entirely on graceful degradation (`ProjectTier.QUALITY` vs `DEVELOPMENT`).
- **The frozen build kills "replace" outright.** `optimized_build.py` contains **zero** references to `pg_dump` (grepped), and the binaries are **never bundled** by standing policy (`sandbox.py`: "on the user's `PATH` (never bundled)"). A packaged Windows install without PostgreSQL client tools has no `pg_dump`, so a pg_dump-only Explorer ships dead in the one artifact end users get.
- **Credentials/connection: solved already, with one new hazard.** `sandbox.py::_pg_env` puts the password in `PGPASSWORD`, never argv, and `clone_data(target_params, sandbox_params, …)` is already the quality-vs-sandbox two-`ConnectionParams` shape. The new hazard is **skew**: a `pg_dump` subprocess is a *second connection at a different instant* than the psycopg introspection that built the tree, so tree and buffer could disagree — impossible today under §18.1's "one fetch path serving two consumers, never a second parallel fetch".
- **Scope of "all ddl issues" — audited, and it is both narrower and wider than claimed.**
  - **`db/migration_gen.py` — pg_dump does NOT help, and this is the one that matters.** It is `DEC-260811094437`'s named trigger and it refuses table/column differences because a migration needs the **`ALTER` from state A to state B**. `pg_dump` has no diff mode; it emits CREATE-from-current only. **So the proposal does not answer the decision it was given as an answer to.** State this plainly to the owner.
  - **`db/deploy_bundle.py` — unaffected.** Pure decision layer over `compute_drift_markers`; renders no DDL.
  - **`db/sandbox.py::build_baseline_sql` / `_build_table_sql` — THE REAL WIN, and nobody named it.** This is a **second** table-DDL renderer already in the tree, it **executes** its output (a genuine consumer, not a view), and it deliberately omits primary keys, foreign keys, `DEFAULT`, indexes, sequences and extensions — meaning today's sandbox tables are structurally unlike quality's. `pg_dump --schema-only | pg_restore` here needs **no spans, no parsing, no navigation**, has a natural fallback (the existing builder) when the binary is absent, and lives in the module that already owns the pg_dump exception. If the owner wants one pg_dump feature, this is the one with the best benefit-to-risk ratio in the tree.
  - **`db/schema_diff.py` / `schema_compare_panel.py` — unaffected/worse**: catalog-based comparison would have to parse dump text to diff it.

**Proposed approach — the owner's dual-mode shape, elaborated. Four parts.**

**Part 1 — the capability probe, on quality connect.** Extend `db/sandbox.py::probe` (which **already** resolves `pg_dump_path` via the injectable `Which` seam and **already** decodes `server_version` from `server_version_num`) with the one missing fact: `pg_dump --version`, run through the **existing injectable `ProcessRunner`** — no new plumbing, and `DATA_CLONE_TOOLS` already names the binary. Add the derived verdict to `SandboxCapabilities` beside `data_clone_available`, e.g. `pg_dump_version: tuple[int, ...] | None` plus a `full_ddl_available` property. Runs **on connecting to the quality server** (owner-settled), and its result is cached on the capability record that the Explorer already has in hand.

**"Correct version", concretely: `pg_dump` **major ≥ server major**.** `pg_dump` dumps happily from an *older* server and **refuses a newer one** (`aborting because of server version mismatch`), so the rule is one comparison — `pg_dump_version[0] >= server_version[0]` — and **newer-than-server is fine, not a mismatch.** The three shapes and their verdicts:

| Shape | Detected by | Mode | Notice |
|---|---|---|---|
| **Absent** from `PATH` (or not installed at all) | `which("pg_dump") is None` | restricted | `[DDL] Restricted DDL — pg_dump not found on PATH; showing DDL reconstructed from pg_catalog (no inheritance/partitioning).` |
| **Present but OLDER than the server** | `pg_dump_version[0] < server_version[0]` | restricted | `[DDL] Restricted DDL — pg_dump 15 is older than server 17; pg_dump would refuse this server. Showing DDL reconstructed from pg_catalog.` |
| **Present, major ≥ server** (incl. newer) | otherwise | **full** | `[DDL] Full DDL via pg_dump 17 (server 16).` |

Every notice names **both versions**, because a version message that omits one of the two numbers is the one that generates the support question. A probe failure (binary present but `--version` errors, times out, or is unparseable) is treated as **absent** — degrade, never guess — and says so.

**⚖ ONE VERSION RULE, NOT TWO — owner, 2026-08-12, and it is a general design principle, not just this feature's answer.** Verbatim: *"yes. anyway the two database (quality and sandbox) must be the same version, and must have the same treatment as we are developing for quality in sandbox"*. Consequences, all settled:
- **The sandbox Explorer (§18.7's second instance) gets full mode too.** Same dual-mode rule, same notice, same fallback — the sandbox receives *the same treatment as quality*, everywhere, as a standing rule rather than a per-feature choice.
- **Quality and sandbox are required to be the same PostgreSQL major.** So a quality/sandbox version divergence is an **error condition to report**, not a case to support. That collapses the version story: **one `pg_dump`-vs-server comparison, not two**, and `pg_dump` major ≥ *the* server major answers for both roles.
- **Probe shape, restated to match:** keep the owner's original *"runs when connected to the quality server"* as the single probe. Do **not** add a second per-instance probe for the sandbox — under one version rule a second check could only ever restate the first, and two checks that can disagree is precisely how a "which one is right?" bug is manufactured. If the sandbox is reached without a quality connection, probe on that connect instead; the rule is **one probe per connected server, one verdict**, not one per Explorer.
- **⚠ FINDING: the same-version requirement is ASSUMED TODAY, NOT ENFORCED — and nothing even checks it.** Verified: `server_version` is read **only** from `db/sandbox.py::probe` into `SandboxCapabilities` and consumed **only** by `db/ddl_check.py::needs_trigger_drop` (gating `CREATE OR REPLACE TRIGGER` at PG 14). **No code anywhere compares a quality server's version to a sandbox server's version** — and §18.5 D2's sandbox is explicitly *bring-your-own local PostgreSQL*, so its major is whatever the user happened to install. The owner's principle is therefore **currently unenforced, unchecked and silently violable**. This feature does not create that gap but is the first to depend on it. **Recommendation: add the comparison as part of this entry** — probe both servers' majors where both are connected and emit a `[DDL]` (or `[Sandbox]`) row when they differ — and if the coordinator prefers it handled independently, it is a legitimate bug report in its own right. Do not silently assume the rule holds.

**Part 2 — the every-open notice, and WHICH surface.** ⚠ **Verified, because the name in the ruling does not exist as a widget.** There is no "Messages **bar**" in this app. What exists:
- **`Messages`** — the bottom dock's **accumulating tab** (`ui/findings_panel.py::RESULTS_TAB_TITLE = "Messages"`, routed by `ui/audit_router.py`'s `TO_RESULTS`; the identifier keeps the old `results` spelling on purpose). This is where `[Check]`, `[Lint]`, `[Validate]`, `[Sandbox]` and `[Schema] VERIFY` rows already land, and it is **visible**.
- **The status bar** — `StaticStatusBar.showMessage` **paints nothing**: it journals into the Activity Log and `displayed_message()` is permanently `""` (stated at `ui/xsd_controller.py:297-306`). Routing this notice there would be `BUG-260812002307` again.
- **The Activity Log** — a journal, not a bar.

**Reading the ruling as the `Messages` tab** — the visible, accumulating bottom-dock tab. It matches every word except "bar", it is where check output already goes, and it is the only one of the three that satisfies *"the choice is clear"*. **Concrete route: a NEW `[DDL]` prefix in `ui/audit_router.py`, mapped to `TO_RESULTS`.** A new prefix is needed because none of the existing ten fits (`[Sandbox]` is the sandbox lane; unprefixed rows already default to `TO_RESULTS`, but an unprefixed row cannot be filtered or recognised). **Owner: confirm `Messages` is what you meant — see open question 1.**

Emitted **on every DDL open** — every Explorer build/refresh in the quality role — accepted repetition, by ruling. In the **restricted** case the row is additionally the natural home for the *reason*, which the in-buffer `RECONSTRUCTION_NOTICE` cannot carry (the buffer notice says *what* is missing; the Messages row says *why this machine is in restricted mode and what to install*).

**Part 3 — full mode: what the buffer is, and what happens to click-to-navigate. ⚖ NAVIGATION IS NOW OWNER-SETTLED** (2026-08-12): *"click should bring to create table."*

- **ONE whole-database `pg_dump --schema-only` per Explorer build — NOT per-table `-t`.** This is the finding that changed between passes and it resolves the worst objection: in a whole-database dump each **`CREATE TABLE … );` is itself contiguous**, and each `ALTER TABLE ONLY … ADD CONSTRAINT` / `CREATE INDEX` / `CREATE SEQUENCE` is a contiguous **standalone statement**. So contiguity does **not** require `-t`, and the 300-subprocesses-per-refresh cost against §18.1's settled **no-cache** posture **disappears** — one subprocess per build, versus one psycopg round trip today. **The `-t` finding stands as a recorded trap**, not as a chosen route: per-object dumping would reintroduce both the spawn cost *and* the dependency-omission problem (`-t` does not dump objects the table depends on, so a `SERIAL` column's owned sequence can go missing — the very case the idea opens with). **Nobody may reach for `-t` later without re-reading this paragraph.**
- **Spans are recovered by PARSING statement boundaries, on the EXISTING `DdlObjectSpan` — no second span type, so §18.1's trap 1 is respected.** `kind="table"` spans the `CREATE TABLE` statement; `kind="column"` detail spans point at their lines *inside* it (one line per column, name-matchable); `kind="constraint"` and `kind="index"` detail spans point at their own standalone statements further down the file. Every tree click still navigates — **navigation is preserved in full**.
- **⚖ CONTAINMENT IS LOST, NAVIGATION IS NOT — and the loss is ACCEPTED, not an open cost** (owner, 2026-08-12: *"click should bring to create table"*). In full mode a table's object span covers its **`CREATE TABLE` statement**, not a region enclosing its constraints and indexes. Two consequences, both accepted: (a) folding a table hides less than it does in restricted mode — **fold behaviour visibly differs between the two modes**, which is a further reason the every-open notice earns its keep; (b) `ddl_editor_panel.py::_span_at_line` on a constraint line resolves to the **constraint**, not the enclosing table. **Do not re-raise either as a cost** — record them as design, and pin the click-lands-on-`CREATE TABLE` behaviour with a test so a later refactor cannot drift it.
- **The parser is a NEW pure, Qt-free module — `db/pg_dump_ddl.py`, beside `table_ddl.py` and `ddl_buffer.py`, for the same reason those two are pure**: dump text in, `(lines, spans)` out. Every branch is then testable against canned fixture text with **no server and no subprocess**, exactly as `table_ddl.py`'s branches are testable without a server today.
- **⚠ The residual risk, and it must be written into the spec rather than left as a footnote: we take a dependency on `pg_dump`'s TEXT LAYOUT, which is not a documented contract** (`pg_catalog` is). The mitigation is that a parse failure **degrades to restricted mode with a named `[DDL]` row** — never a half-parsed buffer, never spans pointing at the wrong lines. Silent wrong navigation is worse than restricted DDL.

**Part 4 — the sandbox renderer, bundled by owner ruling ("keep it in the pg_dump feature").** `db/sandbox.py::build_baseline_sql` / `_build_table_sql` is **a second, lossier table renderer in the same tree, and its output is EXECUTED** (it provisions the sandbox), not merely shown. By its own docstring it deliberately omits **primary keys, foreign keys, `DEFAULT`, indexes, sequences and extensions** — so today's sandbox tables are **structurally unlike quality's**, which is a defect shape this feature is expected to fix, not a stylistic one. When `full_ddl_available`, provision via `pg_dump --schema-only` → `pg_restore`, reusing `clone_data`'s exact shape (`_pg_env`'s `PGPASSWORD`-never-argv discipline, the `ProcessRunner` seam, `CloneDataError`'s named failure). Keep `build_baseline_sql` as the **degrade path** — the same dual-mode rule as the buffer, so there is one rule in this feature, not two. *(Note the ordering constraint that already exists there: `SET check_function_bodies = off` before routines, triggers after routines — a `pg_restore` path must not lose it.)*

**⚖ Part 5 — WHAT THE COMPLETE DDL IS FOR: a CLONE SOURCE, not an editing surface. Owner, 2026-08-12, and it refutes a premise this entry briefly carried.**

Verbatim: *"editing is already present! we can add, alter, delete columns, add index, add constraint... might not be an interface for it, but an educated developer can do all with the menupoints, the sql console. Create table is necessary only for one reason, **to create a clone of a table, or a table similar to it**. Create table ddl is not used to alter. Alter is already present in many ways. so we do have an editor, but educated developer centered not gui"*

**The gesture, concretely:** read a table's full DDL → edit the text by hand (rename it, tweak it) → execute it in the SQL Console → **a new table** that is a clone of, or similar to, the original. **Nobody applies it back to the table it came from.**

**What therefore FALLS AWAY — do not let any of it be re-derived** *(an earlier pass of this entry proposed a split entry, `FQ-260812025836`, built on the wrong premise that "editing a table's DDL" meant a GUI editing surface; that entry is **WITHDRAWN** and its analysis is void)*:
- **No `ALTER` generation, no table diff engine, no `db/migration_gen.py` dependency.** `pg_dump` renders **state**, and state is exactly what a clone needs.
- **No apply-to-existing-table path**, so "what does applying an edited table mean" is a moot question.
- **§18.1's assertion STANDS unchanged:** `Alter Table ▸` remains the only way a table's shape changes, and **`EDITABLE_SPAN_KINDS` need not grow.** Altering is already covered — by the menu and by the SQL Console — for a developer who knows what they are doing.
- **The trigger double-identity collision DISSOLVES.** It only existed because something was to be *checked out and applied*. Nothing is: a cloned table's triggers are just text the developer edits and runs, under a new name. What remains is not a collision but a **completeness** question — are the table's triggers in the text at all (restricted: no; full: yes) — and that is answered by the mode, below.

**Audience, in the owner's own words: *"educated developer centered, not gui"*.** The deliverable is **faithful text a competent developer can wield**, not a guided experience. That is a design constraint with teeth: it argues for verbatim tool output over anything this app re-renders, and it is the strongest possible case for the owner's original `pg_dump` instinct.

**🔴 THIS MAKES RESTRICTED MODE'S INCOMPLETENESS MATERIALLY MORE DANGEROUS, AND IT IS NOW THIS ENTRY'S HEADLINE.** A developer who clones a **partitioned** or **inherited** table from restricted-mode DDL gets a **plain table that looks right** — no `PARTITION BY`, no `INHERITS` — and executes it. That is **a silent wrong result reaching a real database, produced by a human acting in good faith on text the app handed them.** Read merely as a view, the two gaps were disclosed incompleteness; read as a clone template, they are a defect generator. Three consequences:
1. **The every-open `[DDL]` notice is not a nicety — it is the thing that stops someone cloning from a knowingly incomplete template.** Its wording in restricted mode should say so, not merely report a missing binary. Suggested: *"…showing DDL reconstructed from pg_catalog — **incomplete for inheritance/partitioning; do not clone a partitioned or inherited table from this text.**"*
2. **RECOMMENDATION (not a decision): say something stronger at the point of COPYING, not only at open.** A notice read at open is forgotten by the time the user selects text twenty tables down. Options to weigh: keeping the per-table `RECONSTRUCTION_NOTICE` inside the copied region (it already is — it sits directly above each `CREATE TABLE`, so a whole-object copy carries it, which is a point in the current design's favour), or a copy-time `[DDL]` row. **Owner/design call, deliberately not made here.**
3. **`DEC-260811094437`'s gaps get a sharper case for closing than diffing ever gave them** — see below.

**⚖ THE ONE EXCEPTION TO CLONE-ONLY: VIEWS. Owner, 2026-08-12** — *"the exception is view, that can be create or replace, or the ddl can start with drop view, then create view. developers will know this"*. A view's DDL text **is** an alter mechanism, because `CREATE OR REPLACE VIEW` changes the existing view in place, and where that will not do (a changed column list or type) the developer writes `DROP VIEW` then `CREATE VIEW` themselves. **That work is split into `FQ-260812025836`** (re-scoped from the withdrawn table-editing framing to *"make views editable"*), because it is a §18.5 apply-lane feature and needs nothing from `pg_dump`. **This entry stays CLONE-SOURCE-ONLY for tables and matviews.**

**⚠ MATVIEWS ARE NOT VIEWS — state this wherever the rule is written, because everything else in this codebase groups them** (the tree branch, `TableInfo.kind`, `pg_get_viewdef`, `build_view_ddl`'s shared body). **PostgreSQL has no `CREATE OR REPLACE MATERIALIZED VIEW`.** A matview can only be replaced by `DROP` + `CREATE`, which **discards its stored data**, requires a `REFRESH` afterwards, and drops dependents unless handled. So matviews stay **clone-source**, with tables. This carve-out has been widened by analogy twice in this project already (BUG-052, BUG-063); do not let *"views and matviews"* be typed out of habit here.

**⚠ The `CREATE SEQUENCE` question changes too, and it is a concrete, checkable failure mode — in BOTH modes.** A `SERIAL` column renders as `integer DEFAULT nextval('orders_id_seq'::regclass)`. Copy that `CREATE TABLE` under a new name and the clone's default **points at the ORIGINAL table's sequence**: the two tables then draw from one counter, and dropping the original breaks the clone. Restricted mode never emits the owned `CREATE SEQUENCE` at all (by design — §18.1 rules it a separate catalog object). **Full mode emits it, but `pg_dump` puts sequences in a DIFFERENT SECTION of the file from the table**, so a developer copying the `CREATE TABLE` span still does not get it. **So neither mode is clone-safe for `SERIAL` today.** Recommendation: in full mode give the table's owned sequence a **detail span of its own** (`kind="sequence"` on the existing `DdlObjectSpan` — a new kind, not a new type, so trap 1 still holds) so it is navigable and visibly attached to the table; and consider stating the hazard in the per-table notice. **Verify the exact `pg_dump` section placement before designing** — this is the second experiment this entry needs.

**Determinism — what the existing test covers and what it becomes.** §18.1/BUG-018 require the buffer byte-identical across fetches, pinned by a test permuting catalog order. That test **applies to restricted mode only, and stays exactly as it is** — `table_ddl.py` is untouched by this feature and remains pure. Full mode cannot inherit it: no catalog rows are in play, and `pg_dump` output embeds `-- Dumped from database version …` / `-- Dumped by pg_dump version …` plus a version-dependent `SET` preamble, so it is deterministic **for a fixed (server version, pg_dump version) pair** but **not** stable across a client upgrade, and **not** identical between two §18.7 Explorer targets running different majors. What replaces it, and this must be built rather than assumed: (a) a **purity test on `db/pg_dump_ddl.py`** — same dump text in ⇒ identical lines and spans out; (b) a test that **one** `pg_dump` invocation is issued per Explorer build; (c) the spec records that end-to-end byte-identity in full mode is an **environmental assumption, not a proven property** — the honest downgrade, written down instead of discovered.

**Alternatives considered:**
- **REPLACE the synthesizer outright (the idea as first stated).** REJECTED **by the owner's own ruling** — the fallback is what keeps the feature alive in the frozen build (`optimized_build.py` has **zero** `pg_dump` references and the binaries are never bundled by standing policy), so a packaged install without client tools gets restricted DDL instead of a dead pane.
- **A separate non-navigable `Full DDL (pg_dump)` command (this entry's FIRST recommendation).** **SUPERSEDED by the ruling.** Recorded because its reasoning was answered rather than ignored: it avoided the parser entirely, but it left the buffer permanently restricted and made completeness an extra gesture the user has to know about. The owner chose visible-mode-plus-fallback over a second command, and the every-open notice is what makes that safe.
- **Per-table `pg_dump -t` to get contiguity.** REJECTED on two verified grounds (spawn cost against a settled no-cache buffer; `-t` omitting the table's owned sequence). Recorded as a trap, above.
- **A second span type for non-contiguous objects.** REJECTED and **not needed** — parsing statement boundaries gives contiguous spans per statement on the existing `DdlObjectSpan`. §18.1's trap 1 (one dataclass, one `kind` field) holds unchanged. *(If implementation discovers a shape that genuinely cannot be expressed this way, that is an owner question, not an implementer's licence to add one.)*
- **Close the inheritance/partitioning gaps inside `table_ddl.py` (`DEC-260811094437` option 2).** REJECTED — dominated: full mode gets those *and* every future PostgreSQL feature for free, and the synthesizer's gaps stop being a ceiling the moment a correct `pg_dump` is present.

**What this does NOT solve, and the entry must not be read as claiming it — `db/migration_gen.py`.** It refuses table/column differences because a migration needs the **`ALTER` from state A to state B**; `pg_dump` has no diff mode and emits CREATE-from-current only. That is `DEC-260811094437`'s **named trigger**, so this feature **supersedes that decision in direction without answering it**: the migration generator still has no table renderer, and — per the owner's clone-only ruling — **nothing wants one**.

**🔴 But `DEC-260811094437` is RE-POINTED AT THIS ENTRY, and its trigger condition is now shown to be MIS-WORDED.** The condition reads *"the first feature that consumes this buffer as anything other than a read-only view"* — and under the clone framing **the app never consumes it at all; a human does.** Strictly, the trigger never fires; in practice the harm is worse than any diff would have caused, because a developer clones a partitioned table from restricted-mode text and executes it against a real database. **Restate the condition as *"the first feature that INVITES this buffer to be used as a source"*, which cloning plainly is.** *(`owner-decision` owns editing the decision file; this text is the pointer and the finding, not the edit.)*

**Keyboard gate (read `docs/KEYBINDINGS.md` this session):** **No chord, and none may be invented.** Nothing here adds a command form: the mode is chosen by capability, not by the user, and the notice is an audit row. Nothing touches `RESERVED_SEQUENCES` (DEC-014) or any existing binding, and DEC-012 has no application because there is no new command. If a manual *"re-check pg_dump"* action is later wanted, it is a `QAction` with **no** shortcut unless the owner names one for a ledger check.

**Suggested placement:** **EXTEND §18.1** with a new sub-block — *"Full DDL via `pg_dump`, restricted DDL otherwise — the dual-mode buffer"* — placed directly after the `FQ-260810183812` block whose reconstruction boundary it relieves, and **rewriting that block's `⚠ correctness requirement` to be mode-conditional** (the two-gap notice is a *restricted-mode* statement from now on, not an unconditional one). Cross-reference from **§18.5 D2a**, which owns the "pg_dump is a narrowly-scoped exception, never bundled" invariant — **that invariant is widened by this feature and the spec must say so explicitly**, since pg_dump stops being with-data-clone-only. Also fold Part 4 into **§18.5 D2** (`build_baseline_sql`'s provisioning contract) and record against **`DEC-260811094437`** that its direction is superseded but its named trigger is untouched. **CREATE is wrong**: a second DDL-viewing subsection beside §18.1 is the near-duplicate parallel functionality the placement gate exists to prevent. Reuse, do not duplicate: `DATA_CLONE_TOOLS`, `which("pg_dump")`, the `ProcessRunner` seam, `_pg_env`, `MissingCloneToolError`/`CloneDataError`'s named-refusal shapes, and `probe()`'s `server_version`.

**Open questions — ALL OWNER QUESTIONS ARE NOW ANSWERED; only implementation details remain, and none of them block starting.**

1. ~~Is the `Messages` tab what "Messages bar" meant?~~ **ANSWERED 2026-08-12: YES — the bottom dock's accumulating `Messages` tab** (`RESULTS_TAB_TITLE`, routed by `TO_RESULTS`). Proceed with the new **`[DDL]` prefix** in `ui/audit_router.py` mapped to `TO_RESULTS`. **The status-bar reading was put to the owner and explicitly rejected**, and the reason is recorded so nobody re-proposes it: `StaticStatusBar.showMessage` **paints nothing** — it journals into the Activity Log and `displayed_message()` is permanently `""` — so that route would have been `BUG-260812002307` again.
2. ~~Navigation in full mode — is losing containment acceptable?~~ **ANSWERED: yes** — *"click should bring to create table."* Folded into Part 3 as design.
3. ~~Does the sandbox Explorer get full mode?~~ **ANSWERED: yes, and more broadly** — one version rule, sandbox gets the same treatment as quality. Folded into Part 1 as a **standing principle**, not a per-feature answer.
4. ~~Confirm the split.~~ **SETTLED (coordinator, not the owner — queue organisation is not an owner decision).** Split entry is **`FQ-260812025836`**.
5. **(Implementation, verify early.) The `pg_dump` invocation's exact flags** — recommend `--schema-only --no-owner --no-privileges`, and **keep comments** (`--no-comments` would *lose* the `COMMENT ON` lines the synthesizer renders today, making full mode less complete than restricted mode in one respect).
6. **(Implementation.) Timeout and thread.** `pg_dump` on a large schema is seconds; the Explorer build must not block the GUI thread. Reuse the existing off-thread probe pattern (`new_project_dialog.test_sandbox`) rather than inventing one, and give the subprocess a timeout whose expiry degrades to restricted mode with a `[DDL]` row.
7. **(Implementation.) Does the notice repeat on a plain tree refresh, or only on an Explorer open?** The ruling says *"every time DDL is opened"*; if a refresh is not an "open", take the narrower reading and state it in the spec.
8. **(Coordinator.) The unenforced same-version rule** (Part 1's ⚠ finding): fold the quality-vs-sandbox major comparison into this entry, or file it as its own bug? Recommend folding it in — this feature is the first thing that depends on the rule being true.
**⚠ Related entry filed in parallel — `FQ-260812025353` (Project setting: "Locate postgres binaries" folder + server version on both Test buttons).** It replaces PATH-only resolution (`which("pg_dump")`) with a configured folder invoked by full path, empty ⇒ PATH. **That is this entry's Part 1 probe input**, and the two must not grow two different resolution rules: whichever ships first owns binary resolution, and the other consumes it. If `FQ-260812025353` lands first, Part 1 probes the **configured** binary; if this one lands first, it must resolve through a seam that the folder setting can later feed. Its Test-button server-version display is also the natural place to surface Part 1's verdict to the user before they ever open the Explorer.
...
**Problem:** Binary resolution is PATH-only today. `db/sandbox.py::probe` resolves the tools with `which("pg_dump")` / `which("pg_restore")` (`sandbox.py:172-173`, via the injectable `Which` seam at `:75`), and `clone_data` shells out to whatever those resolve to. Two concrete failures follow: on a machine with several PostgreSQL majors installed, PATH picks an arbitrary one — not necessarily the major that matches the quality/sandbox server, and `pg_dump` **refuses a newer server** (`aborting because of server version mismatch`); on Windows the client tools are frequently absent from PATH entirely, so the "with data" clone and the incoming `pg_dump`-based DDL view (FQ-260812022749) simply cannot run. There is also no way to *see* the mismatch: `test_target` reports only `(ok, message)` (`project_settings_dialog.py:457-477`) and `test_sandbox` reports "Connected — superuser." with **no server version** (`_apply_sandbox_probe_result`, `:498-523`), even though `probe` already decodes `server_version` from `server_version_num`.

**Proposed approach:**
- **New per-project field `postgres_bin_dir`** (empty by default) on `ProjectSettings`, serialized as one new key through `db/ddl_project.py::_settings_to_dict` (`:160`) and read back in `_settings_from_dict` (`:197`, which uses `.get()` throughout, so older `settings.json` files load unchanged — same unknown-key tolerance `_parse_sandbox_mode` relies on at `:227`). The store is `<project>/.ddlproject/settings.json` — **plaintext and GITIGNORED** (`ddl_project.py:26-27`, `save_settings` re-asserts the gitignore at `:157`), so a machine-specific absolute path lives per-project and **never travels via git**. This RESOLVES the obvious "machine-specific path in a shareable project" objection — it is not an open question; the existing store is exactly the local-only place such a path belongs.
- **Connection-tab UI:** a **Browse-for-folder** control (folder picker + editable path field) beside the existing connection fields in `project_settings_dialog.py`. Empty ⇒ PATH (today's behavior, bit-for-bit). Set ⇒ full-path invocation of every client tool the app shells out to (`DATA_CLONE_TOOLS = ("pg_dump", "pg_restore")`, `sandbox.py:84`) from that folder.
- **Folder-aware resolver — the one consumer-side change.** Introduce a resolver that, when `postgres_bin_dir` is set, returns `<folder>/pg_dump[.exe]` / `<folder>/pg_restore[.exe]` **if that file exists**, else falls back to `which(name)` — i.e. the folder is consulted FIRST, PATH second, empty-folder = today exactly. Append `.exe` on Windows. Feed this resolver in as the `Which` seam (or a thin wrapper over it) into `probe` and `clone_data`, so no call site outside the resolver needs to know whether the path came from the folder or PATH.
- **Test buttons report the server version.** Append the decoded `server_version` to the status text of BOTH `test_target` (target/quality tester — `SELECT version()` / `SHOW server_version`, since it returns only `(ok, message)` today) and `test_sandbox` (which already has `caps.server_version` from `probe`). Quality and sandbox may differ — surfacing both is the point.
- **Validation when the folder is set (warn, don't block):** verify `pg_dump`/`pg_restore` actually exist in the folder; if not, warn inline rather than hard-blocking Save — the user may be mid-typing a path, and PATH is still a legitimate fallback.
- **RECOMMENDED ADDITION beyond the literal ask (mark for owner confirmation):** when the folder is set, also report the **located binaries' own versions** (`pg_dump --version`) right there in the settings field. That, next to the server version now on the Test buttons, is what makes the "PG18 server vs PG15 pg_dump" mismatch directly visible in one place — the natural completion of the owner's "so the user can see the versions" intent. Flagged as an addition, not assumed.

**Alternatives considered:**
- **Fold this into FQ-260812022749 (DUAL-MODE DDL).** REJECTED as a merge — they are genuinely distinct and both live. FQ-260812022749 owns the `pg_dump` **version probe** (`pg_dump --version` → `pg_dump_version` + a `full_ddl_available` verdict on `SandboxCapabilities`), the version RULE (`pg_dump major ≥ server major`, "newer-than-server is fine"), and the every-open Messages notices. It *assumes* `which("pg_dump")` (PATH-only). **This entry is the missing input that entry assumes away:** the folder field makes that probe's binary resolution folder-aware, so it reports the *located* binary's version, not a PATH one. Keep them separate; cross-reference (see Open questions).
- **An app-wide (not per-project) default binary directory.** DEFERRED — the owner asked for per-project, and per-project is also what the gitignored local store makes clean. A cross-project fallback is a possible future, out of scope for v1; noted so it is not silently dropped.
- **Bundle the client tools with the app.** REJECTED — standing policy is the binaries are **never bundled** (`sandbox.py`: "on the user's `PATH` (never bundled)"; `optimized_build.py` has zero `pg_dump` references, per FQ-260812022749's grep). Locating an existing install is the sanctioned shape; bundling is not.
- **Hard-block Save when the folder lacks the tools.** REJECTED in favor of warn-don't-block (above) — the field is optional and PATH remains a valid fallback.

**Suggested placement:** **EXTEND §18.2 Project settings** — add the `postgres_bin_dir` field (`ProjectSettings` + `settings.json` serialization + Connection-tab Browse-for-folder UI + the two Test buttons reporting server version). The single consumer-side change — folder-aware binary resolution in `db/sandbox.py::probe` and `clone_data` via the existing `Which` seam — is **shared with FQ-260812022749** and must be **cross-referenced** to it: this entry provides the folder-aware resolution seam; FQ-260812022749's `pg_dump_version` probe and version-match RULE/notices consume it and remain FQ-260812022749's domain (do not redefine them here). Whoever folds this dispatches `spec-maintainer` (it harmonizes first, then folds). **CREATE is wrong** — nothing existing covers a binary-location setting, but the field belongs in the established §18.2 settings block and the resolver reuses `db/sandbox.py`'s existing seams (`Which`, `DATA_CLONE_TOOLS`, `_pg_env`, `ProcessRunner`) rather than inventing a parallel path.

**Keyboard gate (read `docs/KEYBINDINGS.md` this session):** **No chord, and none may be invented.** This adds a folder field, a Browse button, and version text on the two existing Test buttons — all dialog widgets, no command form. Confirmed against the register: nothing is added and nothing existing is touched; no `RESERVED_SEQUENCES`, DEC-012, DEC-014 or DEC-015 consideration applies.

**Open questions:**
1. **(Owner — recommended addition, relay.) Show the *located binary's* version (`pg_dump --version`) in the settings field**, next to the server version on Test? The owner asked literally for server version on Test; showing the located binary version too is the natural completion of "so I can see the versions," but it is beyond the literal request — confirm.
2. **(Owner — recommend WARN, not block.) When the folder is set but lacks `pg_dump`/`pg_restore`, warn inline vs. hard-block Save?** Recommendation: warn (PATH remains a valid fallback; the user may be mid-edit). Confirm.
3. **(Future, out of scope for v1.) Any app-wide default binary directory as a cross-project fallback?** Owner asked per-project; noted as a possible future, not built now.
4. **(Coordination — implementation ordering.) FQ-260812022749 shares this resolution seam.** This resolution foundation naturally lands with or just before FQ-260812022749, whose probe must consult `postgres_bin_dir`. Whoever picks up either should coordinate so the seam is folder-aware before the version probe reads it.

---

## FQ-260812025705: Move the three app-wide "Locate external tool binary" settings (PHP Generator, panGen runtime, PHP linter) into the consolidated Software Settings dialog, and grey out each binary's dependent operations when it is unset/invalid
**Status:** PROCESSED (`ui/software_settings_dialog.py::ExternalToolsPane`, commit 179d5e0; 8169 passed / 51 skipped)
**Closing note:** a **sixth** Software settings pane, and the one that diverges from the dialog's apply contract — deliberately. It persists on **Browse**, immediately, through each lane's existing `locate_*` method, which is `ThemesPane`'s *Use this theme* precedent. So it holds **no edit buffer**, and therefore correctly has no OK/Cancel for the host to rebuild on `finished`. That is a real difference from the four original panes and is documented at length rather than left to be discovered.

**MOVE, not duplicate**, per the rule the owner settled for the first four surfaces: the two `Generation` entries and the `Tools` one are gone, with their orphaned separators. Their command ids get **no alias rows** — there is no successor command a pinned toolbar button could fire, so `resolve_ids` drops them, which is the honest degradation.

Each lane re-gates its own actions from inside its own `locate_*`, so the pane never touches a `QAction`; disabled entries name the new address in their tooltips, and the four stale trigger-time notices were re-pointed there. The address is one constant, with a **deliberate literal copy** in `lint/findings.py` — a pure module that must not import `ui/` — pinned by a test.

The per-project PostgreSQL **folder** setting was deliberately not unified with these three: they store full executable paths in `generator_config.json`, so `resolve_tool`'s folder semantics do not apply. What was reused is the seam shape and the warn-don't-block status line.
**Requested:** 2026-08-12
**Idea (verbatim/summarized):** (Owner, over three messages, as an addition to FQ-260812002827.) *"Addition to FQ-260812002827: Locate phpgen binary and PanGen binary should be part of Settings instead of in Generation menu. If no binaries are set, their menupoints should be inactive (greyed out)."* Then: *"same treatment for locate php linter."* So: **move the three app-wide 'Locate external tool binary' settings into the consolidated Software Settings dialog (FQ-260812002827), out of their current menus, and grey out (disable) the operations that depend on each binary when it is not set.**

**Problem:** Three app-wide external-tool "locate the binary" actions live in operation menus today, not in a settings surface, and each dependent operation is always enabled and only fails at trigger-time with an error pointing back at the locate menu item. Concretely (verified this session):
1. **PHP Generator (phpgen)** — `Generation ▸ Locate PHP Generator Executable…` (`ui/generation_controller.py:201`, `locate_generator` :222). Stored app-wide in `generator_config.json` key `executable_path` (`generation/config.py:34` `_EXECUTABLE_KEY`, `load_executable_path`/`save_executable_path` :45/:63, under `_app_data_dir()`). **Dependent op:** `Generate PHP` — always enabled, errors on trigger when unset ("Locate the PHP Generator executable first…", `generation_controller.py:280-285`).
2. **panGen runtime** — `Generation ▸ Locate panGen Runtime…` (`generation_controller.py:210`, `locate_pangen_runtime` :570). Stored in the SAME `generator_config.json`, key `re_phpgen_root` (`generation/config.py:82`, `load_re_phpgen_root`/`save_re_phpgen_root`; validated by `generation/re_runner.py::validate_re_phpgen_root` — root must contain `src/re_phpgen`). **Dependent ops:** `panGen` / `rePHPgen` / `Save reJSON` (gated at `generation_controller.py:377-394`, error-on-trigger when unset/invalid; note `_save_rejson_action` ALREADY toggles enabled at :218/:492/:513 — so proactive gating is not a new pattern, just not applied to the others).
3. **PHP linter** — `Tools ▸ Locate PHP Linter…` (`ui/lint_controller.py:196` `locate_linter`, §22). Path stored app-wide in the SAME `generator_config.json`, key `lint_executable_path` (`lint/config.py`, `load_lint_executable_path`/`save_lint_executable_path`, imported at `lint_controller.py:82`), read INSIDE `LintService` on each lint (docstring :32-36). **Dependent ops:** `Tools ▸ Lint Current File` and `Tools ▸ Lint on Save` (the latter a QSettings toggle, `LINT_ON_SAVE_SETTINGS_KEY="lintOnSave"`, `lint_controller.py:94`). Always enabled; reports "NOT RUN: no PHP linter is configured — use Tools ▸ Locate PHP Linter…" when unset (`lint/findings.py:291-300`).

All three stores are APP-WIDE (`generator_config.json` in the app data dir) — which is exactly why they belong in the app-wide, Maintenance-only Software Settings dialog and NOT in per-project settings (contrast the deliberately-separate sibling FQ-260812025353, below).

**Proposed approach:**
- **ONE new "External tools" pane in the Software Settings dialog (FQ-260812002827, shipped `19c14c5`, `ui/software_settings_dialog.py`).** A new pane is a single `SETTINGS_PANES` row + a pane widget (the shipped dialog's own contract, "adding one later is a single row in `SETTINGS_PANES`, proved by a test that injects a fifth"). The pane holds all three locators — **PHP Generator executable**, **panGen runtime root**, **PHP linter** — each with a Browse control + inline validity indication, reading/writing the EXISTING stores via the EXISTING load/save functions (`load_executable_path`/`save_executable_path`, `load_re_phpgen_root`/`save_re_phpgen_root` + `validate_re_phpgen_root`, `load_lint_executable_path`/`save_lint_executable_path`). **Reuse the stores; do NOT reimplement them.** (Recommend ONE pane over three; strengthened by all three sharing `generator_config.json`. Owner confirms one-vs-three — see Open questions.)
- **RELOCATE = MOVE** (per FQ-260812002827's owner-settled principle *"relocating means moving, so they won't be anymore where they were before."*): REMOVE the three Locate actions from their current menus — `Generation ▸ Locate PHP Generator Executable…`, `Generation ▸ Locate panGen Runtime…`, `Tools ▸ Locate PHP Linter…`. The External-tools pane becomes their SOLE home. The OPERATIONS stay in their menus (Generate PHP, panGen, rePHPgen, Save reJSON, Lint Current File, Lint on Save) — only the LOCATE settings move.
- **Grey out (proactively `setEnabled(False)`) each binary's dependent operations when its binary is unset/invalid**, replacing today's always-enabled-then-error-on-trigger behaviour: `Generate PHP` ⇐ `executable_path`; `panGen`/`rePHPgen`/`Save reJSON` ⇐ a VALID `re_phpgen_root` (`validate_re_phpgen_root`, so an invalid root greys them too, not just an empty one); `Lint Current File` / `Lint on Save` ⇐ linter configured. **The core work is the refresh wiring:** setting/clearing a binary in the pane must update the dependent actions' enabled state LIVE, and the state must also be evaluated at startup. Use the app's injected-callback decoupling idiom — the pane signals the generation lane (`ui/generation_controller.py`) and lint lane (`ui/lint_controller.py`) to re-evaluate; the pane does not reach into their actions directly. `_save_rejson_action`'s existing enable-toggle (`generation_controller.py:218/:492/:513`) is the local precedent to follow and extend to the sibling actions.
- **Update the now-stale error/notice text.** Today's trigger-time errors point at the menu items that this entry REMOVES: `generation_controller.py:285` ("Locate the PHP Generator executable first…"), `:383`/`:394` (panGen/rePHPgen/Save reJSON), and `lint/findings.py:291-300` ("use Tools ▸ Locate PHP Linter…"). With proactive greying these are largely unreachable, but where they can still fire (e.g. an invalid path that passes a shallow check) they must point at **Software Settings ▸ External tools**, not the removed menu paths.

**Design notes / consequences (stated, not open questions):**
- **Maintenance-only consequence — CONSISTENT, flag it.** The Software Settings dialog is Maintenance-only (FQ-027/DEC-006, and FQ-260812002827's `DEC-260812004358`), so SETTING these three binaries becomes Maintenance-only, while the operations themselves run in Project/Standalone mode. A user who has not set a binary sees the operation greyed out and must enter Maintenance mode to set it. This is the SAME owner-accepted trade already made for Customize Shortcuts / Customize Toolbar (FQ-260812002827) and the Themes pane (FQ-260812021716) — *"settings are configured in Maintenance mode."* Record as consistent.
- **The greyed-out action is now the discoverability cue** that replaces today's error-on-trigger — which is also WHY the stale error text (above) matters: it used to be the only pointer to the locate action, and that action has moved.
- **Deliberately SEPARATE from FQ-260812025353 (per-project postgres binaries).** Those are PER-PROJECT (`.ddlproject/settings.json`, gitignored, the version must match the project's DB) and live in Project settings. These three are APP-WIDE (`generator_config.json` in the app data dir) and live in Software Settings. Do NOT conflate or unify them — the app-wide-vs-per-project scope difference is precisely the reason for the split. Cross-reference FQ-260812025353 as the sibling per-project binary-locator so the reader sees the intentional parallel and the intentional divide.

**Alternatives considered:**
- **Leave the three Locate actions in their menus and only add the greying-out.** REJECTED — it ignores the owner's explicit instruction to move them into Settings, and it keeps binary configuration scattered across `Generation` and `Tools` menus rather than in the one settings home FQ-260812002827 exists to be. The greying-out and the move are one coherent change: the greyed action IS the new discoverability cue for a setting that now lives elsewhere.
- **Three separate panes, one per binary.** REJECTED as the recommendation (owner to confirm): the three are all app-wide external-tool locators sharing a single `generator_config.json`, so one "External tools" pane is more cohesive and fewer left-list rows; three panes would fragment a naturally grouped concept. Recommend one; flagged as an owner confirm, not assumed.
- **A hard-block modal on trigger instead of proactive greying.** REJECTED — that is essentially today's behaviour (error-on-trigger) and the owner asked specifically for the menu points to be *inactive (greyed out)*, i.e. proactive disable, which is also the better affordance and matches the existing `_save_rejson_action` precedent.
- **Fold into the app-wide external-tools with the per-project postgres binaries (FQ-260812025353).** REJECTED — different scope (app-wide vs per-project), different store (`generator_config.json` vs `.ddlproject/settings.json`), different settings home (Software Settings vs Project settings). See the Design note above; kept as a cross-reference, not a merge.

**Suggested placement:** **EXTEND the shipped FQ-260812002827 (Software settings)** — add ONE "External tools" pane (one `SETTINGS_PANES` row + pane widget reusing the three existing load/save stores), and REMOVE the three Locate actions from `Generation`/`Tools`. Plus targeted changes in the **generation lane** (`ui/generation_controller.py`) and **lint lane** (`ui/lint_controller.py`) to remove the Locate menu actions and add proactive enable/disable gating with a live refresh hook (injected-callback idiom; also evaluated at startup). Cross-reference **§22** (PHP lint), the **generation section** (§19 vendor generation / §20 re_phpgen), FQ-260812002827 (host dialog), and FQ-260812025353 (sibling per-project binary locator, deliberately separate). Because it MOVES menu actions out of `Generation`/`Tools`, whoever implements it **MUST dispatch `spec-maintainer` (it harmonizes first, then folds) with Supersession Ledger rows** for each relocated action — do not fold silently.

**Keyboard gate (read `docs/KEYBINDINGS.md` this session):** **No chord, and none may be invented.** I read the full register: none of the three Locate actions being removed carries a chord (they have no row — verified), so removing them frees nothing and touches no `RESERVED_SEQUENCES` entry. The new External-tools pane is a Maintenance-only Settings surface and adds NO chord (its `QAction` host, like the parent Software Settings command, carries no `setShortcut` per FQ-027/DEC-006). The dependent OPERATIONS (Generate PHP, panGen, rePHPgen, Save reJSON, Lint Current File, Lint on Save) keep whatever bindings they have — none of them appears in the register today, so proactively toggling their `enabled` state changes no chord (a disabled `QAction` simply does not fire its shortcut, which is the intended effect). No DEC-012/014/015 consideration applies; no ledger row changes.

**Open questions:**
1. **(Owner — recommend ONE pane, confirm.)** One "External tools" pane holding all three locators, or three separate panes? Recommendation: one, since all three are app-wide external-tool locators over a single `generator_config.json`. Confirm.
2. **(Owner — behaviour confirm.)** With proactive greying, is a still-reachable trigger-time error (e.g. a path that existed at set-time but has since been deleted/renamed) acceptable to keep as a secondary guard — re-pointed at Software Settings ▸ External tools — or should the pane re-validate on open and the operations re-evaluate on focus so the greyed state is always current? Recommendation: keep the guard AND re-evaluate live; confirm the extent of live re-validation (e.g. on window focus vs only on pane save).
...
**⚠ THIS ENTRY WAS RE-SCOPED, 2026-08-12, and its previous content is VOID.** It was created hours earlier as *"make TABLES editable — which requires a table diff engine"*. **The owner refuted that premise outright:** *"editing is already present! we can add, alter, delete columns, add index, add constraint… an educated developer can do all with the menupoints, the sql console. Create table is necessary only for one reason, to create a clone of a table, or a table similar to it. Create table ddl is not used to alter."* So a table's complete DDL is a **clone source**, not an editing surface — that belongs to **`FQ-260812022749`** and **no table diff engine is wanted, needed, or in scope anywhere.** Nothing in that withdrawn framing survives: no `ALTER` generation, no `db/migration_gen.py` dependency, no reversal of §18.1's *"`Alter Table ▸` is the only way a table's shape changes"*. This entry is the **one genuine exception** the owner then named: **views**.

**Problem:** A view is the one relation kind whose DDL text really is an alter mechanism — `CREATE OR REPLACE VIEW` changes it in place — yet the Explorer refuses to edit it. `ui/ddl_buffer_panel.py::NOT_EDITABLE_REFUSALS` says *"Views are read-only here — this pane does not edit them"*, because `db/ddl_buffer.py::EDITABLE_SPAN_KINDS` is `{"function", "procedure", "trigger"}`. That refusal was correct when it was written (it was drawn around the *checkout* model), but it lumps views in with tables, and views do not belong there.

**🔴 The headline is the opposite of the withdrawn version's: this is SMALL, because a view already fits the shipped apply lane exactly.** §18.5's `Apply to quality` and the sandbox Check ladder are built around objects with **`CREATE OR REPLACE` semantics** — which is precisely why functions, procedures and triggers are the editable kinds and tables are not. A view has that property. **Verified against the code, not assumed:**
- `ui/ddl_object_editor.py::DdlObjectRef` is `(kind, schema, name, table, arg_types, disambiguate)` and its `key` is `(kind, schema, name, table, arg_types)`. A view slots in as `kind="view"`, `table=None`, `arg_types=()` — **the keying machinery accommodates it with no change.**
- The live source already exists and is already fetched: `TableInfo.view_definition` (`pg_get_viewdef`), which `db/table_ddl.py::build_view_ddl` renders **verbatim** today. No new query.
- `db/apply.py::apply_ddl` executes statements against a target; nothing in it is routine-specific.

**The three concrete places it is NOT free — each verified, each small, each locatable:**
1. **`resolve_edit_target` has no view branch.** It branches `trigger` → `schema.triggers`, else `schema.routines.get(span.signature)`; a view span carries `signature=None`, so it returns `None`. It needs a third branch reading `schema.tables[…].view_definition`.
2. **`DdlObjectRef.qualified` would render a view as `pr.orders_v()`.** It is `f"{schema}.{name}{signature}"` for anything non-trigger, and `signature` is `"(" + ", ".join(arg_types) + ")"` → `"()"` for a view. That string is user-visible (tab tooltip) **and appears in apply confirmations and result headlines** (`f"applied {self._ref.qualified} to …"`), so a confirmation would name a database object that does not exist under that name. **This is the one place the machinery genuinely assumes a routine**, and it needs a third branch, not a tweak.
3. **⚠ The checkout path COLLIDES.** `db/ddl_project.py::routine_ddl_paths` maps a sole-holder routine to `ddl/<schema>.<name>.sql`. A view at `ddl/<schema>.<name>.sql` would **collide with a same-named function** (`pr.orders_v` the view vs `pr.orders_v()` the function — legal in PostgreSQL, and the pattern is common). Worse, §18.2's identity recovery reads the checked-out file's header, and `_HEADER_PREFIX_RE` matches **`CREATE [OR REPLACE] FUNCTION|PROCEDURE` only** — a checked-out view file's identity could not be recovered at all. **So if views join the checkout model, both must change**; if they do not, say so explicitly (open question 2).

**Proposed approach:**
- Add `"view"` to `EDITABLE_SPAN_KINDS`; drop the `"view"` row from `NOT_EDITABLE_REFUSALS`; add the `resolve_edit_target` view branch returning `(DdlObjectRef(kind="view", …), CREATE OR REPLACE VIEW <name> AS <view_definition>)`.
- Give `DdlObjectRef.qualified` (and `short_title`'s sibling logic) a view branch so no `()` is ever rendered for a view.
- **The editable text is built from `pg_get_viewdef`, NOT from `pg_dump` output** — see the trigger question below; this is what keeps the entry independent.
- **Reuse §18.5's apply lane unchanged**: same Save/Apply split, same four preconditions, same confirmations, same sandbox Check ladder.
- **NO GUARDRAILS around `DROP VIEW` + `CREATE VIEW`.** The owner is explicit — *"developers will know this"*. Where `OR REPLACE` is insufficient (a changed column list or type) PostgreSQL raises a clear error and the developer writes the `DROP` themselves. **Do not add a wizard, a detection heuristic, or an offer to drop-and-recreate.** Record this so a later pass does not "improve" it.

**⚠ MATVIEWS ARE EXCLUDED, and the reason must travel with the rule.** **PostgreSQL has no `CREATE OR REPLACE MATERIALIZED VIEW`.** Replacing one means `DROP` + `CREATE`, which **discards its stored data**, requires a `REFRESH` afterwards, and drops dependents unless handled — so a matview has none of the property that makes this feature safe. It stays read-only, with tables. **This is exactly the shape of carve-out this project has had widened by analogy twice in one day (BUG-052, BUG-063)**, and the risk here is unusually high because *everything else in the codebase groups the two*: the tree branch, `TableInfo.kind`, `pg_get_viewdef`, and `build_view_ddl` which renders both. **Whoever implements this must add a test that a matview span is still refused**, not merely refrain from adding it.

**Alternatives considered:**
- **Make tables editable too (this entry's withdrawn scope).** REJECTED by the owner: altering is already covered by `Alter Table ▸` and the SQL Console, and a table's DDL is for cloning.
- **Include matviews "since they're views".** REJECTED — no `CREATE OR REPLACE`; see above. The single most likely mistake in this entry.
- **Build the editable view text from `pg_dump`'s output** (i.e. depend on `FQ-260812022749`). REJECTED, and rejecting it is what makes this feature independent and small: `pg_get_viewdef` is already fetched, already rendered verbatim, and carries **only the view body** — no triggers, no ownership, no `SET` preamble. It is also what today's read-only rendering already shows, so the editable text matches the buffer the user clicked.
- **Leave views read-only and tell users to use the SQL Console.** REJECTED — that is today's behaviour, and the owner has named views as the exception precisely because the console round trip is the friction.

**Dependency:** **NONE.** This entry does **not** depend on `FQ-260812022749`; the two are independent and can be built in either order. *(The withdrawn framing claimed a dependency; that was a consequence of the wrong premise.)*

**Suggested placement:** **EXTEND §18.5** (the DDL object editor, apply & sandbox validation) — the section that owns *what is editable and what apply means* — plus a small amendment to **§18.1** narrowing its read-only assertion from *"tables, views and matviews are not editable"* to *"tables and matviews"*, with the `CREATE OR REPLACE` reason stated. If views join the checkout model it also touches **§18.2** (path scheme + header recovery, see open question 2). `spec-maintainer` should carry a **Supersession Ledger row** for the narrowed refusal set. **CREATE is wrong**: a parallel editing surface would duplicate the app's most safety-critical path. Reuse, do not duplicate: `edit_refusal_for_span`/`NOT_EDITABLE_REFUSALS`, `resolve_edit_target`, §18.5's tab and its Save/Apply split, `db/apply.py`.

**Keyboard gate (read `docs/KEYBINDINGS.md` this session):** **No new chord, and none may be invented.** The feature reuses the existing **`Edit DDL`** context-menu command form; widening which span kinds it accepts changes **no** binding, adds no ledger row, touches no `RESERVED_SEQUENCES` entry (DEC-014) and raises no platform-conditional case (DEC-015). Under **DEC-012** `Edit DDL` keeps exactly one keyboard host and carries no shortcut today.

**Open questions:**
1. **(Design — RE-EXAMINED, and it does NOT block.) The trigger double-identity collision.** It was raised for tables, said to dissolve under the clone framing, and is **live again in principle for views**, since a view can carry `INSTEAD OF` triggers which already have their own `ddl/*.sql` identity (`trigger_ddl_path`, keyed `schema.table.name` where `table` is the view). **It dissolves in practice because of a design choice made above:** the editable text is `pg_get_viewdef`, which contains **only the `SELECT` body** — no `CREATE TRIGGER` statements. So a view's triggers stay editable only as their own objects, exactly as today, and no object gets two checkout identities. **This is load-bearing: if anyone later switches the editable source to `pg_dump`'s output, the collision returns immediately.** Recorded here so that change cannot be made casually.
2. **(Owner/design.) Does an edited view get a `ddl/*.sql` checkout file (§18.2), or is it edit-and-apply only?** If yes, both problems in point 3 above must be solved — a path scheme that cannot collide with a same-named routine, and header recovery that recognises `CREATE [OR REPLACE] VIEW`. If no, say so explicitly and state what Save means for a view. **Recommendation: edit-and-apply only for v1**, which keeps the feature genuinely small and defers the checkout-model widening to a decision of its own.
3. **(Design.) `CREATE OR REPLACE VIEW` vs a leading `DROP VIEW`** — the editable text is offered as `CREATE OR REPLACE VIEW`; the developer may replace it with `DROP` + `CREATE`. Confirm the apply path executes whatever is in the buffer without inspecting it for shape (it should — that is the existing contract), and add no detection.
