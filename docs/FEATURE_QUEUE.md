...
## FQ-260812022749: Source table DDL from `pg_dump` instead of synthesizing it — challenged, and recommended as a SEPARATE non-navigable command rather than a replacement for `table_ddl.py`
**Status:** QUEUED
**Requested:** 2026-08-12
...
**The crux — verified, and it is worse than "pg_dump emits no offsets":**
...
**Proposed approach (recommended): a SEPARATE, explicitly-named command whose output is plain non-navigable text — `table_ddl.py` and the navigable buffer are left exactly as they are.**

- A new read-only **`Full DDL (pg_dump)`** command, reachable from the DDL Explorer object-row context menu (and/or the Deployment menu — owner to choose), which runs `pg_dump --schema-only` for the selected object or schema through the **existing injectable `ProcessRunner`/`Which` seams** and shows the result in a plain read-only tab. **No spans, no folding, no click-to-navigate, no `Edit DDL`** — its honesty is precisely that it is verbatim tool output.
- **Guarded by a named pre-flight**, reusing what exists: `probe()`'s `pg_dump_path`, `probe()`'s `server_version`, and `pg_dump --version`. Missing binary or `pg_dump` older than the server ⇒ a **named, actionable refusal** stating both versions and the `PATH` searched, in `MissingCloneToolError`'s shape. Never a silent fallback and never a partial dump.
- **The synthesizer remains the always-available renderer** and keeps its reconstruction notice — with the notice extended by one line pointing at the new command ("for the complete, tool-generated DDL, use **Full DDL (pg_dump)**"), which is the cheapest possible way to make the two gaps a *choice the user can escape* rather than a ceiling.
- **Separately, and higher-value: file a sibling entry for `pg_dump`-based sandbox provisioning** (§18.5 D2/D2a) to replace `build_baseline_sql`'s deliberately-minimal `_build_table_sql` when the binary is present, keeping the current builder as the degrade path. That is where "simplify many problems" is actually true.

**Alternatives considered:**
- **REPLACE `table_ddl.py` with `pg_dump` (the idea as stated).** REJECTED. It costs click-to-navigate for every table, column, constraint and index — the Explorer's entire purpose — because pg_dump's output is non-contiguous per object and would require a second span type §18.1 explicitly forbids; it makes a shipped feature unavailable in the frozen build, which never has `pg_dump`; it converts a proven-pure determinism test into an environment assumption; and it still does not answer `DEC-260811094437`, since `pg_dump` cannot diff.
- **SUPPLEMENT: pg_dump when available, synthesizer as fallback, both feeding the navigable buffer.** REJECTED, and it is the *most* tempting option, which is why it needs the sharpest rejection: it means the Explorer's buffer has **two different renderings depending on the user's machine**, so a bug report, a screenshot, a fold state and a line number all stop meaning one thing. It also pays the full parsing cost (obstacle 1) *and* keeps the synthesizer, i.e. maximum surface for minimum simplification.
- **Close the inheritance/partitioning gaps in `table_ddl.py` after all (`DEC-260811094437` option 2).** Not rejected outright, but strictly dominated by the recommendation: the separate command gives complete DDL for *every* feature PostgreSQL has or will have, at a fraction of the cost, without touching the buffer.
- **Do nothing; keep the two-gap boundary as `DEC-260811022536` drew it.** Viable and cheap; rejected only because the owner has now asked for an escape hatch and the separate command is a small one.

**Keyboard gate (read `docs/KEYBINDINGS.md` this session):** **No chord proposed, and none may be invented.** The feature has a **command form** (a context-menu and/or menu-bar entry), so under **DEC-012** it gets **exactly one keyboard host, the `QAction`** — and the recommendation is that it carries **no shortcut at all**, matching the other Explorer object-row commands. Nothing here touches `RESERVED_SEQUENCES` or DEC-014. **If the owner wants a chord, they must name it and it must be checked against the ledger before implementation** — do not pick one during the design pass.

**Suggested placement:** **EXTEND §18.1** with a new sub-block — *"Full DDL via `pg_dump` — a separate, non-navigable command"* — placed directly after the `FQ-260810183812` block whose reconstruction boundary it relieves, and cross-referenced from **§18.5 D2a** (the pg_dump exception's home) and from **`DEC-260811094437`** (which this supersedes in direction without answering: the migration-generator trigger is untouched, because `pg_dump` cannot diff). It must reuse, not duplicate: `sandbox.py`'s `Which`/`ProcessRunner` seams, `_pg_env`'s `PGPASSWORD` discipline, `MissingCloneToolError`'s named-refusal shape, and `probe()`'s already-collected `server_version` + `pg_dump_path`. **CREATE is wrong here** — a second DDL-viewing subsection beside §18.1 is exactly the near-duplicate parallel functionality the placement gate exists to prevent. The sandbox-provisioning use is a **separate entry against §18.5 D2**, not part of this one.

**Open questions (owner):**
1. **Does the recommendation hold — separate command, buffer untouched?** Or does the owner still want `pg_dump` to *replace* the buffer's table DDL knowing it costs click-to-navigate for tables, columns, constraints and indexes?
2. **Where does the command live** — Explorer object-row context menu, the `Deployment` menu, or both? And **does it want a chord** (if yes: name it; it will be ledger-checked before implementation)?
3. **Scope per invocation:** the selected object only, the selected schema, or the whole database?
4. **Should the sandbox-provisioning use (`build_baseline_sql` → `pg_dump`/`pg_restore`) be triaged as its own entry now?** This entry judges it the higher-value half of the idea.
5. **`pg_dump` older than the server:** hard refusal (recommended, matching `clone_data`) or a warned best-effort dump?
6. **Verification needed before design:** does `pg_dump --schema-only -t schema.table` include the table's owned `SERIAL` sequence, or drop it? The answer decides whether per-table invocation is viable at all.
