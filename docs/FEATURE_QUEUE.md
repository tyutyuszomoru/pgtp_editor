...
## FQ-260812022749: DUAL-MODE DDL — full `pg_dump` view when a correctly-versioned `pg_dump` is present, the synthesized restricted DDL otherwise, with the mode reported to Messages every time DDL is opened (and the same rule applied to sandbox provisioning)
**Status:** QUEUED
**Requested:** 2026-08-12
...
**Part 3 — full mode: what the buffer is, and what happens to click-to-navigate.** The owner did not address navigation; this is the recommendation, with its cost.
...
- **The one real cost, stated rather than discovered: CONTAINMENT is lost, not navigation.** In full mode a table's object span no longer *contains* its constraints and indexes, so (a) folding a table hides less than it does in restricted mode, and (b) `ddl_editor_panel.py::_span_at_line` on a constraint line resolves to the **constraint**, not the enclosing table. Neither breaks anything shipped — tables are not editable, so `resolve_edit_target` answers *"nothing to edit here"* either way — but **fold behaviour visibly differs between the two modes**, which is a second reason the owner's every-open notice earns its keep.
...
2. **(Owner.) Navigation in full mode: containment is lost, navigation is not.** Every tree click still jumps to its statement, but a table's span no longer contains its constraints/indexes, so folding a table hides less and clicking a constraint line resolves to the constraint. Acceptable?
3. **(Owner.) Does the sandbox Explorer (§18.7's second instance) get full mode too**, judged against the *sandbox* server's version, or is full mode quality-only? The ruling says the probe runs on quality connect and is silent on the sandbox role.
