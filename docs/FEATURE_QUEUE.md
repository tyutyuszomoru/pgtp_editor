...
**⚠ Part 5 — the ruling's SECOND half, and it is a NEW FEATURE, not a clarification. RECOMMENDATION: SPLIT IT OUT into a dependent entry, and this entry does NOT own it.**

Owner, verbatim: *"ddl editing of the table should bring us to a complete ddl with all sons and dependencies (including indexes, fks, pks, constraints, comments)"*.

**🔴 THE ONE THING THE OWNER MUST HEAR: this requires generating `ALTER` statements — a TABLE DIFF ENGINE — which is precisely the capability `pg_dump` does NOT have, and precisely `DEC-260811094437`'s named trigger.** The reasoning, verified:
- **Tables are not editable today, at all.** `db/ddl_buffer.py::EDITABLE_SPAN_KINDS` is `{"function", "procedure", "trigger"}`, and `ui/ddl_buffer_panel.py::NOT_EDITABLE_REFUSALS` states six refusals by name — *"Tables are read-only here — change one with Alter Table ▸"* and the same for view/matview/column/constraint/index. §18.1 asserts flatly that *"the `Alter Table ▸` submenu remains the only way a table's shape changes"*. So this is **not** "show the editor more text"; it is **making tables editable for the first time**, and it **reverses a shipped, spec-recorded assertion**.
- **A table has no `CREATE OR REPLACE`.** §18.5's whole apply lane — `db/apply.py`, `Apply to quality`, the sandbox Check ladder — is built for routines and triggers, whose identity is `(schema, name, arg types)` and whose apply is idempotent replacement. Applying an edited *table* DDL has only four possible meanings, and three are bad:
  1. **Diff the edited DDL against the live table and emit `ALTER`s.** The only correct one. It is a **table diff engine**: `db/migration_gen.py` refuses `object_kind="table"` today (`UnsupportedDifference`; docstring: *"Emit routine and trigger migration SQL — NOT table or column migrations"*), so this feature would have to build it.
  2. **`DROP TABLE … CASCADE` then recreate.** Destroys data and every dependent object. Unacceptable against quality; arguably tolerable only in a sandbox.
  3. **Execute the edited text as written.** `CREATE TABLE` against an existing table simply errors. Not a path.
  4. **Open the complete DDL read-only-ish and REFUSE to apply**, keeping `Alter Table ▸` as the only mutation path — the honest cheap option, but probably not what the owner asked for.
- **It therefore FIRES the trigger condition that `DEC-260811022536` / `DEC-260811094437` recorded**: *"the first feature that consumes this buffer as anything other than a read-only view."* The moment an edited table DDL is an **input**, restricted mode's missing `INHERITS` / `PARTITION BY` stop being disclosed incompleteness and become **a silent wrong result** — the app would diff against a table shape it never read. Those two gaps must be closed, or table editing must be refused in restricted mode.

**Completeness parity — checked against the owner's own list, and the answer is "his five, yes; 'all sons and dependencies', no".**

| Owner's list | Restricted (`table_ddl.py`) | Full (`pg_dump --schema-only`) |
|---|---|---|
| PKs, UNIQUE, CHECK, EXCLUDE, **FKs** | ✅ inline, `pg_get_constraintdef` verbatim | ✅ as `ALTER TABLE ONLY … ADD CONSTRAINT` |
| **indexes** | ✅ standalone `CREATE INDEX` (non-constraint-backed only) | ✅ |
| **comments** | ✅ `COMMENT ON TABLE` / `COMMENT ON COLUMN` | ✅ (unless `--no-comments`, which this entry recommends **against**) |
| *beyond the list:* owned `CREATE SEQUENCE` | ❌ | ✅ |
| *beyond:* triggers on the table | ❌ | ✅ |
| *beyond:* inheritance / partitioning | ❌ (the two stated gaps) | ✅ |
| *beyond:* RLS policies, storage params, tablespaces, collations | ❌ | ✅ |

So **the two modes would present DIFFERENT completeness on an editing surface**, which is far worse than differing on a read-only view — a user edits what they were shown and the app applies against something else. **Recommendation: table editing is offered in FULL MODE ONLY, and refused in restricted mode with a named reason** (reusing `NOT_EDITABLE_REFUSALS`' existing shape). That converts an invisible completeness difference into a visible refusal, which is this project's standing preference.

**One collision nobody has named yet, and it is concrete:** full-mode table DDL **includes the table's `CREATE TRIGGER` statements**, and triggers are **already independently editable** (`EDITABLE_SPAN_KINDS`) and **already have their own `ddl/*.sql` checkout file** under §18.2. Editing a table's "complete DDL" would therefore let the same trigger be edited through two surfaces with two different checkout identities. That must be ruled on — most likely by **excluding triggers from the editable table DDL** — before anything is built.

**Scope judgement (requested): SPLIT.** This entry should own **producing** the complete DDL text (both modes, the probe, the notice, the spans, the sandbox provisioning path). A **separate, dependent entry** should own **editing and applying** it. Reasons:
1. **The difficulty is entirely in the apply path**, not the rendering. Bundling would hide a table diff engine behind a rendering feature — and this entry is otherwise implementable start-to-finish today, while the editing half is blocked on an owner ruling.
2. **They touch disjoint contracts.** This entry touches `db/sandbox.py`, a new `db/pg_dump_ddl.py`, `db/ddl_buffer.py`'s assembly and `ui/audit_router.py`. The editing entry touches `EDITABLE_SPAN_KINDS`, `NOT_EDITABLE_REFUSALS`, §18.5's editable tab, §18.2's checkout model, `db/migration_gen.py` and `db/apply.py` — plus it must reconcile with **FQ-025's `Alter Table ▸`**, which is currently *the* table-mutation path. Two ways to change a table is a product question, not an implementation detail.
3. **The dependency is clean and one-way**: editing needs complete DDL text; producing it needs nothing from editing.
4. **`DEC-260811094437` should be re-pointed at the SPLIT entry, not at this one.** This entry supersedes that decision *in direction*; the split entry is what actually *fires* its trigger.

**Coordinator: please supply an id for the split entry** — I will not invent a timestamp. Its one-line scope: *"Make tables editable from the DDL Explorer, opening the complete DDL (table + PKs/FKs/constraints/indexes/comments), and decide what applying an edited table means — which requires `ALTER` generation, i.e. table support in `db/migration_gen.py`."*
...
**What this does NOT solve, and the entry must not be read as claiming it — `db/migration_gen.py`.** It refuses table/column differences because a migration needs the **`ALTER` from state A to state B**; `pg_dump` has no diff mode and emits CREATE-from-current only. That is `DEC-260811094437`'s **named trigger**, so this feature **supersedes that decision in direction without answering it**: the migration generator still has no table renderer. Whoever folds this must say so in §18.5 rather than let the decision look closed.
...
**Open questions:**
1. **(Owner — relay before implementation.) Is the `Messages` tab what "Messages bar" meant?** There is no widget of that name. The recommendation is the **bottom dock's accumulating `Messages` tab** via a new `[DDL]` audit prefix. The one alternative reading — the status bar — **paints nothing** (`StaticStatusBar.showMessage` journals into the Activity Log and `displayed_message()` is permanently `""`), so routing it there would put the notice where nobody reads it. Confirm, or name the surface you meant.
2. ~~Navigation in full mode — is losing containment acceptable?~~ **ANSWERED 2026-08-12: yes** — *"click should bring to create table."* Folded into Part 3 as design.
3. **(Owner — STILL OPEN, relay.) Does the sandbox Explorer (§18.7's second instance) get full mode too**, judged against the *sandbox* server's version, or is full mode quality-only? The ruling says the probe runs on **quality** connect and is silent on the sandbox role.
3b. **(Coordinator/owner.) Confirm the SPLIT in Part 5** and supply the new entry's id. If the owner instead wants table editing inside *this* entry, it stops being implementable-today: it would block on the apply-semantics ruling and on closing restricted mode's inheritance/partitioning gaps.
4. **(Implementation, verify early.) The `pg_dump` invocation's exact flags** — `--schema-only` plus, presumably, `--no-owner --no-privileges --no-comments`? Owner/deploy noise is not the Explorer's subject, but stripping comments would *lose* the `COMMENT ON` lines the synthesizer renders today. Recommend `--schema-only --no-owner --no-privileges` and **keep** comments.
5. **(Implementation.) Timeout and thread.** `pg_dump` on a large schema is seconds; the Explorer build must not block the GUI thread. Reuse the existing off-thread probe pattern (`new_project_dialog.test_sandbox`) rather than inventing one, and give the subprocess a timeout whose expiry degrades to restricted mode with a `[DDL]` row.
6. **(Implementation.) Does the notice repeat on a plain tree refresh, or only on an Explorer open?** The ruling says *"every time DDL is opened"*; if a refresh is not an "open", pick the narrower reading and say so in the spec.
