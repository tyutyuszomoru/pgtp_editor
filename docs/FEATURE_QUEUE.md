...
**Status:** PARTIALLY PROCESSED (4bc73b6) — the DISCOVERABILITY half is done: a visible "Deploy this edit…" button (the row is no longer sandbox-seam-gated) plus an always-visible Database-menu entry, and the picker's prose now names unavailable destinations and why. That work exposed a real bug: `save_requested` was NEVER CONNECTED, so choosing "Save (for a future batch deploy)" silently wrote nothing — fixed and tested.
