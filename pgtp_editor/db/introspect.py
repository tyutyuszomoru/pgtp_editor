...
class ColumnInfo:
    ...
    #: `pg_attribute.attidentity`, normalized: `"a"` for `GENERATED ALWAYS AS
    #: IDENTITY`, `"d"` for `GENERATED BY DEFAULT AS IDENTITY`, `None` for a
    #: column that is not an identity column (the catalog's `''`).
    #: `DEC-260811022536` -- the synthesized `CREATE TABLE` renders this, so it
    #: has to be read here; **`ColumnInfo` grows fields rather than forking into
    #: a sibling type**, the same discipline `DdlObjectSpan` follows. Trailing
    #: and defaulted, so every existing positional or keyword `ColumnInfo(...)`
    #: construction across the codebase and tests stays valid and simply yields
    #: "not an identity column".
    identity: str | None = None
    #: `pg_attribute.attgenerated`, normalized: `"s"` for a `GENERATED ALWAYS AS
    #: (<expr>) STORED` column, `None` otherwise. **The expression itself
    #: arrives in `default`** -- PostgreSQL keeps a generated column's
    #: expression in `pg_attrdef`, the very row `pg_get_expr` already reads for
    #: defaults -- so this flag says how to *read* `default`, and no additional
    #: catalog value is needed for it.
...
# Widened (2026-08-11, `DEC-260811022536`) with an EIGHTH and NINTH selected
# value, additively and on the SAME round trip: `attidentity` and
# `attgenerated`, so `db/table_ddl.py` can render `GENERATED ... AS IDENTITY`
# and `GENERATED ALWAYS AS (<expr>) STORED` instead of omitting them. A second
# query was explicitly not an option (`FQ-260810183812`'s one-round-trip rule,
# the same reason view definitions rode along on an existing query), and both
# values live on `pg_attribute` -- the table this query already reads -- so the
# widening costs nothing but two columns. The first seven values keep their
# meaning and position, and `_build_tables` unpacks the row tolerantly, so the
# many canned 7-tuple column rows across the suite keep working.
#
# A generated column's *expression* is deliberately NOT selected separately:
# PostgreSQL stores it in `pg_attrdef`, so `pg_get_expr(d.adbin, d.adrelid)`
# above already returns it in position 6 (see `ColumnInfo.generated`).
_COLUMNS_SQL = """
...
def _build_tables(
    ...
    # Unpacked tolerantly (`*rest`) for the same reason the relation and
    # constraint rows are: the 2026-08-11 widening appended `attidentity` and
    # `attgenerated`, and the suite is full of canned 6- and 7-value column
    # rows that must keep yielding a plain column rather than raising.
    for schema_name, rel_name, col_name, data_type, notnull, default, *rest in column_rows:
        table_key = f"{schema_name}.{rel_name}"
        if table_key not in columns_by_table:
            continue
        comment = rest[0] if rest else None
        # `''` is the catalog's "not an identity / not generated"; normalize it
        # to None so consumers test one thing, not two.
        identity = (rest[1] if len(rest) > 1 else None) or None
        generated = (rest[2] if len(rest) > 2 else None) or None
